...
import type { FloorProps } from './props.js';
 ...
 * decision — the dispatcher's — and a column-per-subject layout makes you rebuild
 * it every time you look. Everything else is the detail that picture cannot hold.
 *
 ...
 * findings, recovery, the drawer, the world. The floor owns what it draws and
 * decides nothing.
 */
export function FactoryRoot({ view, actions }: FloorProps) {
  ...
  //
  // `Bench` is the floor's own drawing — everything else here is a machine with a
  // lamp and a plate, and the shared cards dropped into a factory card read as a
  ...
  const goalFloor = (
    <section className="fx-card fx-bev" data-fx="goal-floor">
      ...
      <GoalFloor
        ...
        onDismissRun={(issueNumber) => actions.dismissRun(issueNumber)}
