...
function Row({
  ...
  return (
    <div className={`fx-part${inHand ? ' hand' : ''}${tone}${position === undefined ? '' : ' rung'}`}>
      ...
      {/* The watch/ignore toggle — the same label write `WorldSummary` carries,
          drawn here too because the floor puts PRs on the rack instead of in The
          Yard, so this is their only home. Disabled with no
