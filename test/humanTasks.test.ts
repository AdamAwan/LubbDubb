...
test('a human task is created, listed, settled — and survives a restart', () => {
  ...
  const { task } = first.humanTasks.recordHumanTask({
  ...
  assert.equal(first.humanTasks.listHumanTasks().length, 1);
  first.close();

  const second = new Store(dbPath);
  const reopened = second.humanTasks.listHumanTasks();
  assert.equal(reopened.length, 1);
  assert.equal(reopened[0]!.title, 'Rotate the CI deploy key');
  assert.equal(reopened[0]!.status, 'open');

  const settled = second.humanTasks.settleHumanTask(task.id, 'done', 'Rotated and redeployed.');
  assert.equal(settled?.status, 'done');
  assert.equal(settled?.resolution, 'Rotated and redeployed.');
  assert.ok(settled?.resolvedAt);
  assert.equal(second.humanTasks.settleHumanTask(task.id, 'declined', 'no'), null);
  second.close();
});
...
test('a repeat refreshes the row rather than filing it twice, and never resets the status', () => {
  ...
  const first = system.store.humanTasks.recordHumanTask({
  ...
  const again = system.store.humanTasks.recordHumanTask({
    title: 'Enable the webhook',
  ...
  assert.equal(system.store.humanTasks.listHumanTasks().length, 1);

  system.store.humanTasks.settleHumanTask(first.task.id, 'declined', 'not until the migration lands');
  ...
  assert.equal(system.store.humanTasks.getHumanTask(first.task.id)!.status, 'declined');
});
...
test('nothing in the dispatcher reads human tasks — a standalone one blocks nothing', async () => {
  const system = build();
  system.store.world.setWorldBaseline(world([issue(12)]));
  const agent = pickupAgent(system);
  await callTool(system, agent, 'request_human_task', { title: 'Look at the rendered screen' });

  const before = system.store.humanTasks.listHumanTasks();
...
test('a plan step for a person is never dispatched, and holds what depends on it', async () => {
  ...
  const { plan } = ingestPlanDocument(system.store, {
    doc: {
      ...
      parts: [
        ...
        {
          slug: 'verify',
          title: 'Assert on the delivered event',
          scope: 'test/',
          touches: [],
          atoms: [],
          dependsOn: ['webhook'],
  ...
  const backing = system.store.humanTasks.listHumanTasksForParts([step.id]);
  ...
  const dispatched = system.store.tasks
    .listTasks()
    .map((t) => t.originRef)
  ...
  const closed = system.store.plans.listPlanParts(plan.id).find((p) => p.slug === 'webhook')!;
  assert.equal(closed.status, 'concluded');
  assert.equal(closed.outcomeKind, 'human');

  await system.harness.runCycle('manual');
  const after = system.store.plans.listPlanParts(plan.id).find((p) => p.id === dependent.id)!;
...
test('declining a step blocks it rather than concluding it, so nothing downstream starts', async () => {
  ...
  const { plan } = ingestPlanDocument(system.store, {
    doc: {
      ...
      parts: [
        ...
        { slug: 'verify', title: 'Assert on it', scope: 'test/', touches: [], atoms: [], dependsOn: ['flip'] },
      ],
    },
    originRef: 'issue:12',
    title: 'Issue 12',
  });
  const step = system.store.plans.listPlanParts(plan.id).find((p) => p.slug === 'flip')!;
  const backing = system.store.humanTasks.listHumanTasksForParts([step.id])[0]!;
  ...
  assert.equal(system.store.humanTasks.getHumanTask(backing.id)!.status, 'declined');

  await system.harness.runCycle('manual');
  const after = system.store.plans.listPlanParts(plan.id);
...
test('an operator files and settles one through the routes, and the snapshot ships it', async () => {
  ...
  assert.equal(done.statusCode, 200);
  assert.equal(system.store.humanTasks.getHumanTask(humanTask.id)!.status, 'done');
...
test('a settled task is dismissed off the bench; an open one cannot be, and nothing else moves', async () => {
  ...
  assert.equal(system.store.humanTasks.getHumanTask(humanTask.id)!.dismissedAt, null);
  ...
  const row = system.store.humanTasks.getHumanTask(humanTask.id)!;
  ...
  assert.equal(again.statusCode, 409);
  assert.equal(system.store.humanTasks.getHumanTask(humanTask.id)!.dismissedAt, row.dismissedAt);
