...
const ROLES: Record<string, 'work' | 'evidence' | 'deliberation' | 'unrecognised'> = {
  ...
  'issue:12:describe-check:44': 'evidence',
  ...
  'issue:12:criteria-alignment': 'deliberation',
  'issue:12:prediction-judge': 'deliberation',
...
const MINTED: Record<string, string> = {
  ...
  criteriaAlignment: issueOriginRef('criteriaAlignment', 12),
  predictionJudge: issueOriginRef('predictionJudge', 12),
  ...
  describeCheck: issueOriginRef('describeCheck', 12, 44),
...
test('each origin family mints exactly the string it has always minted', async () => {
  assert.deepEqual(MINTED, {
    ...
    criteriaAlignment: 'issue:12:criteria-alignment',
    predictionJudge: 'issue:12:prediction-judge',
    ...
    describeCheck: 'issue:12:describe-check:44',
