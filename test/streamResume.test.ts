...
test('a restored stream agent re-attaches to the same session in the same worktree', async () => {
  ...
  assert.equal(s1.store.agents.getAgent(id)!.status, 'interrupted');
  const before = s1.store.transcripts.getTranscript(id);
  ...
  assert.equal(s2.store.agents.getAgent(id)!.status, 'running');
  assert.ok(s2.agents.isLive(id));
  assert.equal(s2.store.agents.countLiveAgents(), 1);
  assert.ok(
  ...
  const after = s2.store.transcripts.getTranscript(id);
...
test('a crashed stream agent (row still live) is offered restore on the next boot', async () => {
  ...
  assert.equal(s1.store.agents.getAgent(agent.id)!.status, 'running');
  s1.store.close();
  ...
  assert.equal(s2.store.agents.getAgent(agent.id)!.status, 'running');
  s2.store.close();
});
...
test('a parked stream agent is restored still parked, and its escalation is answerable', async () => {
  ...
  assert.equal(s1.store.agents.getAgent(agent.id)!.status, 'waiting');
  assert.equal(s1.store.escalations.listOpenEscalations().length, 1);
  ...
  assert.equal(s2.store.agents.getAgent(agent.id)!.status, 'waiting');
  const open = s2.store.escalations.listOpenEscalations();
...
test('an agent whose process dies mid-run is re-attached to the same session', async () => {
  ...
  assert.equal(system.store.agents.listAgents().length, 1, 'the crash does not mint a second agent row');
  ...
  assert.equal(system.store.tasks.getTask(agent.taskId)!.status, 'running', 'the task is not settled');
  assert.deepEqual(system.store.errors.listErrors(), [], 'a recovered crash is not an error the operator must read');
...
test('an agent that crashes past agentResumeAttempts fails, naming how many resumes were tried', async () => {
  ...
  assert.equal(system.store.agents.getAgent(agent.id)!.status, 'running');
  assert.equal(system.store.agents.getAgent(agent.id)!.resumeAttempts, 2);
  assert.equal(launches.length, 3, 'one launch, then one per resume');

  launches[2]!.child.crash();
  const dead = system.store.agents.getAgent(agent.id)!;
  assert.equal(dead.status, 'failed');
  assert.notEqual(dead.endedAt, null);
  assert.equal(system.agents.isLive(agent.id), false);
  assert.equal(system.store.tasks.getTask(agent.taskId)!.status, 'failed');
  assert.equal(launches.length, 3, 'and nothing relaunches past the bound');

  const errors = system.store.errors.listErrors().filter((e) => e.message.includes(agent.id));
...
test('the resume budget is read off the row, so a harness restart does not refill it', async () => {
  ...
  l1[0]!.child.crash();
  assert.equal(s1.store.agents.getAgent(agent.id)!.resumeAttempts, 1, 'the budget is spent');
  s1.agents.interruptAll();
  s1.store.close();

  const { launches, system: s2, crashed } = reboot(dir, { agentResumeAttempts: 1 });
  assert.equal(s2.store.agents.getAgent(agent.id)!.resumeAttempts, 1, 'the count survived the restart');
  assert.equal(s2.recovery.decide(crashed[0]!.taskId, 'restore').ok, true);

  launches[0]!.child.crash();
  assert.equal(s2.store.agents.getAgent(agent.id)!.status, 'failed');
...
test('agentResumeAttempts 0 keeps a mid-run crash terminal', async () => {
  ...
  launches[0]!.child.crash();
  assert.equal(system.store.agents.getAgent(agent.id)!.status, 'failed');
...
test('a cockpit kill and a clean done stay terminal, whatever the budget says', async () => {
  ...
  assert.equal(system.store.agents.getAgent(agent.id)!.resumeAttempts, 0);
  ...
  assert.equal(s2.store.agents.getAgent(a2.id)!.status, 'done');
...
test('a mid-run resume revokes the dead launch credential and drops its spool', () => {
  ...
  assert.equal(store.agents.getAgent(agent.id)!.status, 'running', 're-attached rather than settled');
...
test('a cockpit kill is NOT offered for recovery on the next boot', async () => {
  ...
  assert.equal(s1.store.agents.getAgent(agent.id)!.status, 'killed');
  s1.store.close();

  const { launches, system: s2, crashed } = reboot(dir);
  assert.equal(crashed.length, 0, 'killed agents are not candidates');
  assert.equal(launches.length, 0, 'nothing is re-spawned');
  assert.equal(s2.store.agents.getAgent(agent.id)!.status, 'killed', 'the kill stays dead');
  s2.store.close();
});
...
test('an orphan with no usable session id is offered requeue and remove, not restore', () => {
  ...
  const task = system.store.tasks.createTask({ kind: 'code', title: 't', prompt: 'p', branch: 'b', originRef: 'r' });
  system.store.tasks.updateTask(task.id, { status: 'running' });
  const agent = system.store.agents.createAgent({
    taskId: task.id,
    cwd: dir,
    pid: 1,
    status: 'running',
  ...
  assert.equal(system.store.agents.getAgent(agent.id)!.status, 'crashed', 'a refusal leaves the decision outstanding');
...
test('detection is idempotent: a second detect does not re-park or double-count', async () => {
  ...
  const taskId = s2.store.agents.getAgent(agent.id)!.taskId;
