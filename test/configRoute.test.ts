...
import { loadConfigFromText } from '../src/config/config.js';
...
test('a save overrides a project value locally and leaves the project’s file alone', async () => {
  ...
  const shownFirst = (await read(system)).groups
    ...
    .find((entry) => entry.path === 'maxConcurrentAgents');
  assert.equal(shownFirst?.value, 4, 'the team’s value is what the harness booted on');
  assert.equal(shownFirst?.isDefault, true);

  const revision = (await read(system)).revision;
  const saved = await save(system, { set: { maxConcurrentAgents: 6 }, baseline: revision });
  assert.equal(saved.status, 200);

  assert.match(readFileSync(file, 'utf8'), /"maxConcurrentAgents": 6/);
  ...
  const shown = (await read(system)).groups
    .flatMap((group) => group.entries)
    .find((entry) => entry.path === 'maxConcurrentAgents');
  assert.equal(shown?.value, 6, 'a live key, so the override is in force now');
