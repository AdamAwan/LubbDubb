...
| `headSha?`           | The commit the checks ran against — GitHub's `head.sha`, Azure's `lastMergeSourceCommit`. The one thing that tells a check that was **fixed** from one that **flaked**: red then green is a push on a different commit and a flake on the same one. Absent means the provider does not report it, and every reader must then stay silent rather than guess → [27](27-knowledge.md#what-the-harness-raises). |
