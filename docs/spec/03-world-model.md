...
`closedPullRequests` is a **window**, `closedPrWindowMs` wide, and it is meant to be: everything in the
harness that reads it is asking about _recently_. What has to outlive it is kept elsewhere — a merge in
the work graph, and the pull request's last reading in `pr_archive`, which is what the cockpit's goal
page draws its closed rows from ([14](14-persistence.md#the-closed-pull-request-archive)).
...
- **PRs the fleet may not act on are removed** and handed over separately as `hiddenPrs` — the ones
  carrying no `-watch` tag, and the ones a colleague opened
  ([07](07-pull-requests.md#whose-pull-request-is-it)) — so nothing acts on one while gates that must
  not read "absent" as "merged" can still see it.
...
| `headSha?`           | The commit the checks ran against — GitHub's `head.sha`, Azure's `lastMergeSourceCommit`. The one thing that tells a check that was **fixed** from one that **flaked**: red then green is a push on a different commit and a flake on the same one. Absent means the provider does not report it, and every reader must then stay silent rather than guess → [27](27-obstacles.md#the-harness-is-a-voice). |
| `unresolvedComments` | `PrComment[]` — review comments waiting on the author, each with `handled`.                                                                                                                                                                                                                                                                                                                                |
| `reviewThreads?`     | `PrReviewThread[]` — the same threads with their replies, where they hang in the diff, and one of four states (`open`/`answered`/`resolved`/`reopened`). `unresolvedComments` is a **fold** of this, so the two cannot disagree. Absent means the provider does not report threads, which every surface must say rather than draw as "no review" → [07](07-pull-requests.md#review-threads).               |
...
| `viewerAuthored?`    | That the credential the harness posts under **opened** this pull request, resolved by the provider against the viewer identity and never against `filters.prAuthor`. `false` is what takes a pull request out of the dispatch world; absent means the provider cannot say, and the readers fall back to the branch shape ([07](07-pull-requests.md#whose-pull-request-is-it)).                             |
| `viewerAssignment?`  | `assignee` \| `reviewer-required` \| `reviewer-optional` — that a **person** put this pull request on the operator, resolved by the provider against `userId`. The one field about a pull request with no rule behind it: it decides a court ([07](07-pull-requests.md#a-pull-request-a-person-put-on-you)) and nothing else. Absent means nothing is assigned **or** the provider does not resolve it.    |
| `author?`            | Who opened the pull request, as the provider names them **to a person** — Azure's `displayName` (falling back to the UPN), GitHub's login. Read by one surface: the row that says a colleague asked you for a review ([07](07-pull-requests.md#a-pull-request-a-person-put-on-you)). Absent means the provider does not report it, and every sentence drops the name rather than inventing one.            |
| `viewerApproved?`    | That **the operator personally** has given this pull request an approving verdict — their own vote, never `approved`, which is any reviewer's. The one thing that ends an assignment ([07](07-pull-requests.md#when-the-assignment-ends)). Absent means the provider did not say, and is never read as a verdict.                                                                                          |
| `assignees?`         | Who the tracker says is on this pull request, as `{id, name}`: GitHub assignees, and Azure reviewers named individually. `id` is what an assignment is written with. Read by one thing, the assign ask ([07](07-pull-requests.md#asking-who-should-look-at-it)). Absent means the provider did not say, never "nobody".                                                                                    |
...
**On the wire the same stub is marked, not dropped.** `/api/state` ships the retained runs beside
`world.issues` as `retainedRuns`, and each carries a `stale` field no live issue has: when the harness
last saw the item in the open set, and — where the ticket mirror holds the item — the tracker's own word
for it now (`Resolved`, `Closed`, open with the watch tag gone). The reading the marking qualifies is the
tracker's copy: title, body, labels, `workItemState`. Everything else on the goal is the harness's own
record — its run, plan, pull requests, spend, pad and instructions — and is as current as any live
goal's, which is why the cockpit keeps drawing the goal rather than losing it the moment the tracker
does ([17](17-cockpit.md#the-goal-page)).
