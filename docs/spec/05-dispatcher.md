...
| `obstacle-repair`          | Something is blocking the fleet      | —                    | A standing obstacle is blocking the fleet **now** — a base branch red, or three or more independent voices — and nothing owns it. One code agent, on origin `obstacle:<id>`, bounded to one repair in flight across the fleet. → [32](32-obstacles.md#ownership) |
