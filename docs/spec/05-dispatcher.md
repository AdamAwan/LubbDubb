...
- **`evidence`** — `assess`, `retro`, `describe:<pr>`, `describe-check:<pr>`, `validate-plan`, `validate:<check>`,
...
| `local-validation`         | Validate a goal on this machine          | —                    | An operator pressed Validate locally, so the harness brought that goal's code up in the machine's dev environment and one agent writes a test plan, drives the running application through it and reports. Dispatched while the environment is still coming up, read-only and pinned to the commit it stands at, with a browser as a second MCP server. → [32](32-local-validation.md)                                          |
...
| `pr-describe`              | Write a description handed back          | —                    | The operator pressed **Hand it to the agent** on an open part's pull request whose agent sent no draft. One read-only agent on `issue:<n>:describe:<pr>` reads the diff and writes the body through `pr_describe`, under `open_pr`'s rules. Only on a press. → [07](07-pull-requests.md#the-agents-draft)                                                                                                                       |
| `pr-description-check`     | Check the description the operator wrote | —                    | The operator wrote or rewrote an open part's pull-request description and nothing has checked that version. One read-only agent on `issue:<n>:describe-check:<pr>` reads it against the diff and reports findings — never text — through `description_review`. Not picky; a contradiction raises an ask, gaps alone a low-priority note. → [07](07-pull-requests.md#every-description-is-checked-without-asking)                |
...
| `prediction-judge`         | Second reading of a prediction           | —                    | The operator has marked moment one on a goal and no judge has: one sealed desk agent on `issue:<n>:prediction-judge` marks the same slots. Once per goal. → [14](14-persistence.md#the-prediction-judge)                                                                                                                                                                                                                        |
| `plan-amendment`           | A change to a plan needs approval        | —                    | A plan that is **running** has an amendment pending against it and no card up for that amendment. Touches the plan not at all; claims no headroom.                                                                                                                                                                                                                                                                              |
| `plan-blocked`             | Approved plan is going nowhere           | —                    | A released plan has something blocked and nothing moving, so nothing will be dispatched for it — and a live part nobody refused is stuck behind it. Asks a human once; dispatches nobody.                                                                                                                                                                                                                                       |
...
group — which also keeps the group contiguous.

`pr-split` is not in it either, and for a different reason: it takes a read-only checkout of its own
under `split/pr/<n>` rather than the pull request's branch, so it is not competing for the branch the
...
`pr-description-check` follows it on the same terms, under `describe-check/pr/<n>`.
...
**One consequence for `pr-review`**, the only rule in the group with an `enabled` condition: the walk
cannot switch a single rule inside a shared pass off, so what the condition holds is what the pipeline
and the cockpit _advertise_, and `needsFleetReview` is what actually holds the concern. Both read
`review.enabled` — one field under both, so they cannot disagree.

**The fleet's own review leads them all** ([07](07-pull-requests.md#the-fleet-review)), and stands
down entirely — rather than ranking below — while a human reviewer has unhandled threads open: the
diff is about to be rewritten, so a second opinion on the old one is spent for nothing.
...
A third source joins those two: the `origin_ref` of every live **ejection**
(`DispatchContext.ejections`), where an operator has taken a running agent's work into their own Claude
Code and the harness is holding the issue for them. It folds in here for the reason the standing jobs
do — a hold that every rule has to remember to honour is one a future rule will not.
→ [35](35-ejection.md#holding-the-origin)
...
9. **Handed-over validation checks** (`validate-check`), below every rule that produces work.
   Validation's standing promise is that it blocks nothing, so a check must never take the final slot
   from a blocked part or a red build — that would make the one feature that gates nothing the reason
...
`issuePickupStates`, and only for items carrying a native `workItemState`. It never fires on a closed
item or on a retained run.

- Item in an effective pickup state, **not already in** `issueInProgressState`, with **no open PR**,
  **no plan** (`partsPlanFor` is null) and a **live work task** on it → move it to
...
rather than an implementation detail. The obvious alternative is to stamp the move where a pickup
candidate becomes a dispatch, at the headroom cut — but that cut is generic, fires exactly once, and
fires for candidates that are then held or superseded. A provider write refused there would never be
retried, and a candidate cut for want of headroom would park an item in "Doing" nobody is working.
Observing costs one pulse of lag and is idempotent and self-healing instead.

It is **mutually exclusive with `work-item-in-review` by construction**: that rule owns an item with
an open PR _or_ a decomposed one, and this one refuses both. If the two ever overlapped the item would
ping-pong, one provider write per pulse, forever — so the exclusivity is a test rather than a comment.

A decomposed item is the review state's business for the whole life of its plan, even though agents
plainly are working: `work-item-in-review` parks it there deliberately, and since the in-progress
state is folded into the pickup set, a rule that disagreed would flip the item back every cycle.
...
`issueInProgressState` is still one this rule can advance when its PR opens. Without the fold it
would be stranded there: outside the pickup list, so never picked up again and never moved on.
...
- The prompt is `validation-plan`, and the plan's hint, what any `coverage` part built and what each
  configured environment can drive are **appended** to it rather than interpolated.
- Fails silent: an agent that crashes or spends its attempt cap leaves the goal exactly as delivered,
...
is left. What the summary _is_ and how it is drawn is [17](17-cockpit.md#the-feature-summary); the
...
- and, with `review.blocking` on, the fleet has reviewed it — a row in `pr_reviews`, whatever the
  verdict said ([07](07-pull-requests.md#the-merge-gate)). Unknown is never clear.
...
`pr-review-triage`, `pr-split`, `pr-describe`, `pr-description-check`, `pr-review`, `pr-review-comment`, `pr-concern-escalation`, `pr-title`, `finding-ticket`, `raise-bug`,
...
when _not_ to set it: a defence of an approach the reviewer may still reject is their thread to
