...
| `pr-describe`              | Write a description handed back      | —                    | The operator pressed **Hand it to the agent** on an open part's pull request whose agent sent no draft (`manualDescriptions` on). One read-only agent on `issue:<n>:describe:<pr>` reads the diff and writes the body through `pr_describe`, under `open_pr`'s rules with the key off. Only on a press. → [07](07-pull-requests.md#the-agents-draft)                                                                            |
