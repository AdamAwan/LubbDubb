...
| `review.backfill`                     | `boolean`                                  | `false`                              | Whether switching `review.enabled` on reviews the pull requests **already open**. **Off**, for `review.enabled`'s own reason one step later: a team with twenty open pull requests means "review what we open from here", and would otherwise get twenty agents on one pulse and — with `blocking` — twenty pull requests it cannot merge, all of it indistinguishable from the feature working. Off, the intake ledger stamps every open pull request the first time the review sees it and only the ones the harness watched appear are eligible ([07](07-pull-requests.md#the-backfill-guard)). Read live, so turning it on later still reaches the pull requests already stamped as backlog. |
