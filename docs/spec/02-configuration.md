...
| `review.charterFile`                  | `string \| null`      | `null`                               | A file in the repository saying what this project asks its reviewers to look at, resolved against `repoRoot` and **appended** to the `pr-review` prompt. Read from the working tree rather than from the branch under review — a pull request that could edit the file it is reviewed against is a gate that reviews whatever it is told to — and read once at boot, so an edit is a restart rather than a pulse. A path that names nothing is recorded on the error log rather than swallowed.                                                                                                                                                                                                                                                                                                                                                                       |
