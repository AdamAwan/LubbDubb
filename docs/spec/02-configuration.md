...
and now on by default — and the entry still refuses. It was a block carrying a confidence _threshold_, and that number is the
...
| `lessonBlockChars`, `knowledgeBlockChars`                              | nothing is injected fleet-wide any more ([27](27-obstacles.md#what-the-claim-store-left-behind))                                   |
...
form decides anything for itself. The reason is that the answer is a fact about the _edit_: the key
...
resolved `required: boolean` would carry the answer for the config the harness is _running_, which is
...
| `localValidation.*`            | Assign **onto `running.localValidation`**, `localRun`'s arrangement and for its reason: the rule reads the policy through a thunk on every decision and the snapshot reads it at every poll, so the object _is_ the late reader. Live because a validation that came back `blocked` because it could not sign in should be answerable by correcting a text box and pressing the button again — not by restarting the harness. → [32](32-local-validation.md#configuration)                               |
...
| `featureBoard`                 | Assign. `system.ts` wires everything the summariser reads — the rule's `featureStandings`, the dossier, the digest a submission is stamped with — as thunks over `featureSummariesOn(config, connector)`, and `/api/state` asks the same predicate every poll, so the object _is_ the late reader. Live because the Features tab's banner turns it on from a button ([17](17-cockpit.md#the-summaries-banner)), and a button that answered _restart to apply_ would be a settings page with extra steps. |
...
| ------------------------------ | --------- | --------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `heartbeatIntervalMs`          | `number`  | `30000`                     | Gap between timer-driven cycles **while the fleet is busy** — a live agent, a queued job, a candidate the fleet itself is holding, or a build in flight. → [the cadence keys](#the-cadence-keys)                                                                                                                                                                                                                                                                                 |
| `idleHeartbeatIntervalMs`      | `number`  | `300000`                    | The gap while none of that holds. A value below `heartbeatIntervalMs` is read as equal to it.                                                                                                                                                                                                                                                                                                                                                                                    |
| `hotReadMaxAgeMs`              | `number`  | `60000`                     | How long a per-entity reading is reused for something that is **moving**, when nothing on the cheap list payload says it changed. Zero re-reads every pulse. → [04](04-harness-cycle.md#hot-and-cold)                                                                                                                                                                                                                                                                            |
| `coldReadMaxAgeMs`             | `number`  | `300000`                    | The same for everything else. A cold item is listed, fresh and in the world every pulse; it is only re-hydrated on this clock. The main lever on what a fast pulse costs the provider.                                                                                                                                                                                                                                                                                           |
...
| `sendPrRepliesWithoutApproval` | `boolean` | `true`                      | Send a review reply the fleet drafted straight to the thread, without asking you. **Opt-out**, and on because that is already what an agent did — it posted to the thread itself, unsigned and unrecorded. What changes is who sends it, not whether it goes. `false` is the stricter setting: every draft then waits in the inbox as a proposal. Replies only — a merge is authorized per pull request by landing a stack. → [09](09-execution.md#the-two-standing-authorities) |
...
Four numbers decide what the pulse costs, and they are four rather than one because they answer
different questions. They arrived together, at the end of the work that made a quiet pulse cheap, so
that the surface could be designed once instead of accreting a knob per stage.

- **`heartbeatIntervalMs`** is the busy pulse: how quickly the fleet reacts to the outside world. It
  was five minutes when every cycle began with a full provider fan-out, and 30s is what the
  change-gated read and the lane split buy.
- **`idleHeartbeatIntervalMs`** is what a fleet with nothing in flight costs while it waits for
  something to happen. Five minutes, which is the old heartbeat: an idle deployment is exactly as
  expensive as the whole harness used to be.
- **`hotReadMaxAgeMs` / `coldReadMaxAgeMs`** are the two lane backstops
  ([04](04-harness-cycle.md#hot-and-cold)). They are not intervals the harness ticks on: each is a
  bound on how old a reading may be before the next pulse pays to refresh it, asked per entity.
...
There is no per-pulse **request budget** key, and no budget mechanism behind one. The reasoning is in
[15](15-integrations.md#is-a-per-pulse-budget-worth-building).
...
| `prewarmWorktrees` | `boolean` | `true`                  | Ready the pool slot a queued branch will want between cycles, instead of on the executor's serial loop. Only ever for a branch that already exists and only into a spare slot, so it brings a checkout forward in time and never into existence. → [09](09-execution.md#warming-a-slot-ahead-of-the-dispatch)                                                                                                                                                             |
...
| `closedPrCatchUpMs`           | `number`        | `604800000` (7d)    | The furthest back a closed-pull-request read may reach to catch up on closes missed while the harness was down. The window above is the ordinary reach; this caps the extra. → [15](15-integrations.md#the-closed-pull-request-read)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |     |
| `obstacleDormantMs`           | `number`        | `604800000` (7d)    | How long an obstacle nobody re-reports and nothing owns stays on the board before it goes `dormant` — decay, the one of the four endings that covers what no reading can see. Its keys survive, so a matching report reopens the row at `standing` with its whole history rather than filing a second one; the number is chosen against the other cost, that a genuinely live obstacle nobody happened to hit over a weekend is still there on Monday. → [27](27-obstacles.md#how-an-obstacle-ends)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |     |
| `obstacleTicketApproval`      | `boolean`       | `true`              | Whether the bug the harness wants filed for a standing obstacle waits for the operator's answer on the Obstacles tab. On, the pulse proposes and a person says _file the bug_ or _no bug_; off, the pulse files it itself, which is what it did before this key existed. The gate is a column on the row rather than a state, so a proposal nobody answers still reaches agents, still resolves when the world clears it and still decays — and the repair door, which is the fleet stopped now, is never gated. → [27](27-obstacles.md#the-ticket-is-proposed-never-filed)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |     |
| `environments`                | `list`          | `[]`                | Where landed work travels — `{name, at, group?, health?, arrival?, watch?}` per entry: a command printing the commit the environment is **at**, optionally a command answering whether the environment is **well** right now (`{"state": "Healthy"}` / `{"state": "NotHealthy", "tier": "Red", "reasons": [...]}`), optionally what arriving there opens on the bench and whether it is said on the ticket, and optionally how its telemetry is asked a declared question — `{observe, schema?, describe?, forMs?, holds?}`, where `holds` is the off-by-default opt-in that withholds a delivered goal's `validate` or `close_out` row until this environment's watch on it has settled. Optionally a `group`, naming the one place several environments are — three regions of production are three `at` commands and one band, and a band arrives, opens a gate and is said on the ticket only when every environment in it holds the work. Empty is the off switch. `fileOnly`: each entry is a shell command the harness runs. → [24](24-environments.md#configuring-an-environment), [24](24-environments.md#groups), [29](29-post-deploy-watch.md#configuring-an-environment) |     |
...
| `environmentHealthIntervalMs` | `number`        | `300000` (5m)       | How often each environment's own `health` command is asked whether it is well. Its own key rather than `environmentProbeIntervalMs`: a probe is asked only while some landing is unconfirmed, and this is asked on every environment that declares a check whether or not anything has shipped — so this number is the standing cost of the feature, and how stale the worst reading on the glass may be. Nothing is asked where no environment declares `health`. → [24](24-environments.md#is-the-environment-well)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |     |
| `watchIntervalMs`             | `number`        | `1800000` (30m)     | How often an **open** post-deploy watch asks its environment again. Deliberately not `environmentProbeIntervalMs`: five minutes is right for a local ancestry question and absurd for a percentile over a 24-hour lookback. Nothing is asked when no watch is open, which is the steady state. → [29](29-post-deploy-watch.md#cost)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |     |
...
| Key                    | Type                     | Default                                                                                                                 | Behaviour                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ---------------------- | ------------------------ | ----------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `userId`               | `string` (optional)      | unset                                                                                                                   | Who _you_ are to every provider — see [`userId`](#userid). Drives ticket assignment, and is who the filters below narrow to. Not branch names, which are operator-neutral. Unset, filed tickets go unassigned and nothing can be filtered.                                                                                                                                                                                                                        |
...
| `issueCompletedState`  | `string` (optional)      | unset                                                                                                                   | The state a work item moves to when the harness **closes** it as done — the close-out row's own button, and nothing else. Unset = the tracker has no close the harness can write, and the row asks the operator to close it there. Azure only; see [closing an item](#closing-an-item).                                                                                                                                                                           |
| `issueNotPlannedState` | `string` (optional)      | unset                                                                                                                   | The state a work item moves to when it is closed as **not planned** — the plan back-out. Unset and the back-out leaves the item open and says so. Azure only; see [closing an item](#closing-an-item).                                                                                                                                                                                                                                                            |
...
| `issueParentedTypes`   | `string[]`               | `["User Story", "Story", "Product Backlog Item", "Requirement", "Bug", "Tech Debt", "Technical Debt", "Debt", "Issue"]` | Item types expected to **hang off** one of the above. One of these with no parent is an orphan: the appraisal prompt says so and offers the containers the harness can see, and the cockpit asks where it belongs. Anything else (a Task under a story) is never asked. Matched case-insensitively; `[]` turns the orphan report off. **List your process template's own names** — a type missing here raises no missing-parent question, ever, with nothing red. |
...
GitHub closes an issue and has no workflow state at all. **Azure has a dozen states and no generic
close**, and which of them means _done_ and which means _we are not doing this_ belongs to the
...
The two are independent: a deployment that names only `issueCompletedState` gets the close-out button
and a back-out that still reports the item left open. A close for a reason whose state is unnamed is
**refused with that key's name** rather than approximated with the other one's word — a story quietly
closed as "Done" when the operator said it was never going to be built is a board that lies about why.

Neither state may also appear in `issuePickupStates`, and a config that does both is **refused at
load**: an item the harness closed into a state it also picks up from reads as eligible on the next
pulse, so the fleet picks up the work it has just finished.
...
| Key                                   | Type                                       | Default                                | Behaviour                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ------------------------------------- | ------------------------------------------ | -------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `featureBoard`                        | `boolean`                                  | `false`                                | Switch on the [feature summaries](17-cockpit.md#the-feature-summary) — rule `feature-summary` ([05](05-dispatcher.md)), which spends one desk agent per Feature whose work has moved to write down where that Feature has got to. **It no longer gates the board**: the Features tab is drawn wherever the provider can place a work item (`canPlaceWorkItem`), flag or no flag, and with the flag off it carries a banner saying what turning this on adds ([17](17-cockpit.md#the-two-gates)). It needs that provider too — on a flat tracker nothing is summarised. Off by default because it costs agents: turned on, every Feature is summarised once straight away, since none has an account yet. With it off the harness does not read the mirror to find out whether anything moved. The key kept its name so deployments that set it keep their summaries.                                                                                                                                                                                        |
...
| `planning.fileBudget`                 | `number`                                   | `20`                                   | Changed files past which a pull request is asked whether it is one piece of work or several. The number the planner and every part agent are told before any code exists, and the trigger for rule `pr-split` after it does. Judgement rather than a limit: nothing is refused, blocked or reshaped for being wide, and what decides a split is a model reading the diff. `0` turns both halves off, and a provider that does not report a file count fires neither. → [07](07-pull-requests.md#how-wide-a-pull-request-is)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `review.enabled`                      | `boolean`                                  | `false`                                | The fleet review ([07](07-pull-requests.md#the-fleet-review)) — the harness reads a pull request of its own before a person is asked to. **Off by default** because it is the one rule that spends an agent on _every_ pull request; a deployment that took the defaults and found its bill changed by a build it had not asked anything of would be right to call that a fault. On, rule `pr-review` leads the PR concerns and the merge gate learns a clause.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `review.blocking`                     | `boolean`                                  | `true`                                 | Whether a pull request nobody has reviewed is held out of rule `pr-merge-ready`. It asks whether the review _happened_, never whether it was clear — with one round nothing could clear a `findings` verdict, and a gate that read it would wedge every pull request the reviewer had an opinion about. Off records the verdict and gates nothing, which is the week a team spends reading what the reviewer says before it starts blocking. Unknown is never clear.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `review.allowSkip`                    | `boolean`                                  | `false`                                | Whether rule `pr-review-triage` may answer that a pull request needs **no review at all** — a version bump, a regenerated lockfile, a typo in a comment. **Off**, because it is the one answer the triage can give that waives the gate rather than sizing it, and with `blocking` on it is also what lets the merge through: a project asks for it or the tool does not carry the argument and the prompt does not mention it. On, it also **turns the triage on by itself** — one declared mode plus a skip is two answers. Never the fail-open direction: silence is a review ([07](07-pull-requests.md#skipping-a-review-altogether)).                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `review.reviewedElsewhere`            | `string \| null`                           | `null`                                 | A command asking whether a pull request has **already been reviewed somewhere else** — an Azure branch policy with a required approver, a review bot, another org's gate. `pr_reviews` can only answer whether the _fleet_ read it, which on a team that already has a reviewer is the wrong question. Run in a shell in `repoRoot` with `LUBBDUBB_PR` set, and **the exit code is the answer** (0 = already reviewed), because what an operator reaches for here already exits 0 for yes. A check that could not answer — missing, killed — leaves the fleet reviewing and goes on the error log: folding it into "already reviewed" would switch the feature off on exactly the deployments whose gate broke ([07](07-pull-requests.md#a-review-that-happened-somewhere-else)).                                                                                                                                                                                                                                                                           |
| `review.publish`                      | `'none' \| 'comment' \| 'tooling'`         | `'none'`                               | Whether the reviewer is told to post what it found on the pull request. `'comment'` names `reply_to_review` and only that: the executor authorises and signs what goes out, where an agent posting from its own shell writes under the operator's own name with nothing recording it. A published finding lands as an unhandled thread, which rule `pr-review-comment` already answers. `'tooling'` is for a project whose **own** review tooling owns the pull request surface — the charter tells the agent to run it, and it threads the findings and posts whatever status the branch policy wants. The note there asks for no comment of the agent's own and, deliberately, forbids no posting: `'none'` under such a charter reads as a contradiction the agent cannot satisfy, and it answers by running the tooling in dry-run, so the status never posts and the merge gate never clears. Pairs with `publishedThreadProperty`, which is the same arrangement on the state-tracking side.                                                          |
| `review.publishedThreadProperty`      | `string \| null`                           | `null`                                 | The key a thread the project's **own** review tooling opened stamps itself with, in the provider's per-thread property bag (Azure DevOps has one; GitHub does not). Declared, the review mark's `addressed` gains a second arm: at least one stamped thread, and every one of them resolved. It exists because `publish: 'none'` — the default — means the harness posts nothing, so `pr_reviews.published_thread` is null on every review and the first arm can never fire: a project publishing findings itself had the mark stay red over resolved threads forever, with nothing red to say why. Null asks nothing and leaves the arm off ([07](07-pull-requests.md#a-thread-the-harness-stamped)).                                                                                                                                                                                                                                                                                                                                                      |
| `review.machineAuthors`               | `string[]`                                 | `['\\[bot\\]$']`                       | Author names that are **machines**, as regular expressions over the login the provider reports. The third and last source behind `authorKind` ([18](18-observability.md#telling-a-person-from-a-machine)): the provider owns up to its own apps, `publishedThreadProperty` catches the project's own tooling, and this catches what neither can see — a service account on a personal access token, a review bot commenting under an ordinary user. Belongs in `lubbdubb.project.json` beside `publishedThreadProperty`, because which machines comment on a repository is a fact about that repository and a committed file means every clone reads the same list. Read at request time, never stored against a row, so a name added next month is applied to every thread already recorded. An author matching nothing here reads as a **person** — an assumption the reading makes visible by drawing authors by name rather than only as a total.                                                                                                       |
| `reviewAreas`                         | `{area, path}[]`                           | `ui`, `sql`, `backend`, `test`         | Which part of the code a review thread was about, worked out from the file it is anchored to. Each row is an area name and a regular expression over the repository-relative path. **A path may match several and is counted under each** — a test for UI code is both, and a single area per file would force a false choice — so the rows of the reading do not sum to its total. A path matching none is `unplaced` and a thread anchored to no file at all is `unanchored`; neither is folded into an area, because a residual quietly inflated is a reading with nothing red. A pattern that will not compile is skipped rather than thrown, since the rules are read on every request and one bad row must not take the panel down. The defaults describe **this** repository's layout and are the one part of the reading a deployment is expected to replace. Read at request time and never stored against a row, so a corrected rule corrects the whole back-catalogue ([18](18-observability.md#the-area-is-derived-at-read-time-never-stored)). |
| `review.publishedThreadRole`          | `string \| null`                           | `null`                                 | Which stamped threads count: the value required on the companion key `"<publishedThreadProperty>.role"`, derived from the property name rather than configured separately so the two cannot name different bags. Null takes **every** stamped thread — which on a poster that also opens an unresolvable summary thread never reads as addressed at all.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `review.modes`                        | `Record<string, {charterFile?, profile?}>` | `{}`                                   | The ways this project reviews, keyed by the names its routing charter uses. Each carries a `charterFile` (what that mode looks for) and a `profile` (what it is worth spending). **Two or more switches rule `pr-review-triage` on**; fewer is no routing at all, because a decision with one option is not a decision. That is the whole switch — a separate flag could disagree with the modes and one of them would be ignored silently.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `review.defaultMode`                  | `string \| null`                           | `null`                                 | The mode a review runs in when nothing routed it — a triage that crashed, was killed or spent its cap. This is the **fail-open** target, so it should name the thorough mode; null takes the first declared. A name that is not a declared mode is **refused at load**: it is only ever reached on a day something else has already gone wrong, so a typo in it looks correct for as long as the harness is working.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `review.routingCharterFile`           | `string \| null`                           | `null`                                 | The file saying **how to choose** between the modes, read by the triage agent. Prose rather than a threshold because the decision is not one: "under three files" is a proxy for risk, and what actually makes a diff worth a careful read is judged rather than counted. Appended and read on a mode charter's terms.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| `localRun.resumeInstruction`          | `text`                                     | `''`                                   | What a session bringing an **interrupted** run back is told. A third field rather than a re-run of `instruction`, because a start is handed a machine with nothing of this project up and a resume is handed one where the last session was reaped mid-flight — the containers it left are still running, and attaching to them is a different sentence from bringing a second stack up beside them. Only the project knows which of its pieces survive a reap. **Empty means an interrupted run settles the way it always did**, with the note saying nothing was configured to bring it back. **Live.** → [23](23-local-runs.md#coming-back-after-a-restart)                                                                                                                                                                                                                                                                                                                                                                                              |
| `localRun.resumeWindowMs`             | `number`                                   | `7200000` (2h)                         | How long after the harness went down an interrupted run may still be brought back. A resume is for a **restart** — an Apply, a Ctrl-C, an upgrade handoff — where the operator wants their environment back in a minute and the containers the reap could not touch are still up. A harness that was off overnight has neither, and the boot would spend a session on an environment nobody asked for and nobody is watching. Judged on the run's `interruptedAt`, stamped by the fast stop; a row nothing stamped is **unknown rather than recent** and is not brought back whatever this says. `0` means no bound, which is the behaviour before there was one. **Live.** → [23](23-local-runs.md#coming-back-after-a-restart)                                                                                                                                                                                                                                                                                                                            |
| `localRun.refreshInstruction`         | `text`                                     | `''`                                   | What the session holding a **running** environment is told after the harness has moved its checkout to the tip of its branch — the project's own steps for picking new code up: a rebuild, a migration, a restart of one service. A fourth sentence beside start, stop and continue because it is a fourth job; the harness does the git half and appends what moved, and only the project knows what that requires. **Empty is a supported state**, not a missing one — a hot-reloading dev server needs nothing said, and the session is still told what moved. **Live.** → [23](23-local-runs.md#refreshing-the-code-under-a-running-environment)                                                                                                                                                                                                                                                                                                                                                                                                        |
| `localValidation.instruction`         | `text`                                     | `''`                                   | What a validating agent is told about reaching _this_ environment — which URL is which application, which account to sign in as and how, what a first load does that looks like a failure, which parts of the stack to leave alone. `localRun.instruction`'s sibling one step later: that one says how the environment is _started_, this one how it is _used_. **Never a secret** — it is readable in the cockpit and a project layer commits it to the repository. Empty is supported: a deployment with nothing to know still hands the agent the URL, the plan and the browser. **Live**, because it is what an operator corrects between one validation and the next. → [32](32-local-validation.md#configuration)                                                                                                                                                                                                                                                                                                                                     |
| `localValidation.browser`             | `ExtraMcpServer \| null`                   | Playwright, headed, persistent profile | The MCP server that gives a validating agent a browser. A server rather than a flag because there is none inside a headless `claude -p`: Claude in Chrome and computer use both require an interactive session. `{outputDir}` and `{profileDir}` in its `args` are filled in per run. **One block, two dispatches** — a local validation and a remote validation sheet's own run both launch it, the latter with its own artefact directory and a profile per environment. **`null` is a real configuration** — an API-only project validates through the API and the logs, and the prompt says so rather than letting the agent invent a screen. **Live.** → [32](32-local-validation.md#the-browser), [36](36-remote-validation.md#the-browser-the-run-drives)                                                                                                                                                                                                                                                                                            |
| `localRun.url`                        | `string`                                   | `''`                                   | Where the application lands once it is up, drawn as a link beside the run — and, since the watch, probed: the panel says whether its port answered. Declared rather than detected: matching a URL in output whose shape is every framework's own is wrong often enough that a dead link beside a working run is the likelier outcome. Frozen onto the row at each start, so a later edit does not rewrite what a past run reported. **Live.**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| `spendBurn.floorSteps`                | `number`                                   | `80`                                   | Absolute tool-using steps a run must **also** have taken, so four times the median of a rule that takes a handful of steps is not an alarm.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `spendBurn.ceilingSteps`              | `number \| null`                           | `null`                                 | A flat per-run step ceiling, fired with no history whatever. `null` = no such arm, for `ceilingUsd`'s reason.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `spendBurn.floorMinutes`              | `number`                                   | `15`                                   | Absolute wall-clock minutes a run must **also** have been going, so four times the median of a rule that finishes in seconds is not an alarm.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `spendBurn.ceilingMinutes`            | `number \| null`                           | `60`                                   | Wall-clock minutes a run may go before it surfaces whatever its comparables say. **The one ceiling on by default**: an hour is a number an operator can hold an opinion about without knowing the work, and it is what lets the watch say anything on a deployment too young to have medians.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| `selfUpdate.checkIntervalMs` | `number` | `900000` | A floor on how often the remote is touched, not on how fresh the served answer is: the reading is held in memory and served from there in between. Fifteen minutes rather than an hour, because the reading is now raised as an ask on the rail and an ask an hour behind the world is one an operator learns to skim. |
...
| `selfUpdate.projectAutoPull` | `boolean` | `true` | Fast-forward the **worked** checkout on its own whenever it cleanly can. Unlike `autoUpdate` this restarts nothing and interrupts nothing, which is why it defaults the other way. Off asks on the rail instead. |
| `selfUpdate.snoozeMs` | `number` | `1800000` | How long `Snooze` hides an update ask on the rail. One window for both asks, so the two cannot drift into meaning different lengths of "not now". |
...
`autoUpdate` is the separate question of whether the harness _takes_ what it finds, and it is off
...
`projectAutoPull` reads as the same permission and is not, which is why it is on by default where
`autoUpdate` is off. That one stops a fleet, ends a process and brings the deployment back on different
code; this one fast-forwards the operator's clone and is refused outright on any checkout where that
would not be clean. What it prevents is the failure this group's own project layer makes possible — a
harness running a `lubbdubb.project.json` the team changed days ago, on exactly the deployment nobody
is watching. → [21](21-self-update.md#pulling-it-without-being-asked)

#### `spendBurn`

Every other cost reading is a post-mortem. This one is taken while the run is still going: a live run
is compared against the median of settled runs of **its own kind of work** — the dispatch rule _and_
the profile, since a pinned goal legitimately costs several times the same rule on a cheap profile —
and one far past that becomes a `burn` human task.

It reads **three axes**: spend, tool-using steps, and wall-clock runtime. Spend alone cannot see the
run this exists to catch — it is only reported when a **turn** ends, so a run grinding away inside one
long turn reports nothing at all — and a cheap profile's runaway is cheap, so it would not clear a
money threshold even reporting perfectly. Steps and runtime are both readable mid-turn.

It **holds nothing and kills nothing**. An expensive run is not a wrong run, and no threshold can
tell the two apart; the notice is the prompt to open the transcript and decide, and where the ladder
has a rung above the run's profile the notice names it so lifting is one press.

Three gates must hold together on each axis (`multiple`, `minimumRuns`, and that axis's floor)
because a multiple on its own fires constantly. Each axis also has a flat ceiling that fires with no
history at all; **`ceilingMinutes` is the only one on by default**, because an hour of wall-clock is a
number an operator can hold an opinion about without knowing the work, while the right number of
dollars or steps is a property of the deployment. A policy that would file constantly — a `multiple`
at or below 1, a `minimumRuns` of 0, a negative floor — is refused at load, naming the key. **The mock
...
| `agentAllowedTools`    | `string[]`                      | JS toolchain + git + gh | Tool allow rules merged into `--settings` as `permissions.allow` (Claude Code syntax, e.g. `Bash(npm:*)`). Pre-approves the mechanical validate/commit/push commands so the default config completes a task unattended without `bypassPermissions`. Never on `--allowedTools` (that carries the MCP grants). Also names the non-Bash tools the CLI gates and an agent routinely needs, `Skill` first among them: a slash command is a gated tool call, so a fleet without that rule parks on the backstop every time an agent invokes one. Default: `Bash(npm:*)`, `Bash(npx:*)`, `Bash(pnpm:*)`, `Bash(yarn:*)`, `Bash(node:*)`, `Bash(git:*)`, `Bash(gh:*)`, `Read`, `Glob`, `Grep`, `Skill`, `WebFetch`, `WebSearch`.                                                                                                                                                                                                                                                                                                             |
...
| `agentSubmitDelayMs`   | `number`                        | `60`                    | `raw` only: gap between writing message text and writing the submitting carriage return.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `agentWaitingPatterns` | `string[]`                      | `[]`                    | Extra literal substrings a terminal session treats as "waiting for input".                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `agentStallNudges`     | `number`                        | `2`                     | How many times an agent that ends a turn with **no** sentinel in it is asked to account for itself before the stop is put to a human. A whole-life budget per agent; `0` parks on the first one. Stream runtime only, which is every runtime that runs a model. → [10](10-agent-runtimes.md#the-unannounced-stop)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
- **A rule points at a named profile, and a profile is a model, the depth it runs at, and the
  ...
  follows. A profile carries nothing beyond those fields — no extra args. `claudeArgs` stays the single
  global escape hatch, which structurally removes the risk of a profile's args clobbering the
  `--allowedTools` MCP grants; every field a profile does carry is either a flag the harness emits
  itself or a posture the harness reads, which is what keeps them out of that argument.
...
- **`autoApprove` is optional, defaults to `false`, and it is the one field that removes a person from
  the loop.** It is there because the same models that cannot take `auto` fall back to `acceptEdits`,
  which auto-accepts file edits and nothing else — so on a `fast` profile every `git`, `gh` or `npm`
  call the allow-list does not name reaches the backstop and blocks on the operator, which on a
  mechanical profile is most of the work ([10](10-agent-runtimes.md#permission-model-issue-130)). Set
  on a profile, `PermissionDesk.request` answers `allow` for that profile's agents itself, raising no
  escalation. It is resolved at dispatch and stored on the task row beside the mode, so a
  boot-`resume` keeps the posture the conversation started on and a profile edited mid-run does not
  change what a live agent may do.
- **What `autoApprove` grants is not what the allow-list grants, and the difference is the point.**
  The allow-list names commands; this names an agent and allows it whatever it asks for, in a worktree
  of the real repo with the operator's shell environment inherited — so it is `bypassPermissions` in
  everything but the root refusal, scoped to one profile. It belongs on a profile whose work is
  mechanical and whose model is cheap, and the reason it is per profile rather than fleet-wide is that
  the deep profiles, which write the code that matters, keep the person at the desk.
...
**Branch names are operator-neutral — `userId` is not in them.** `issueBranch`
(`src/dispatcher/issuePickup.ts`) is `issue/<n>` and nothing else, because the name is not a label
but a _key_: `openPrForIssue` matches a pull request back to its issue by that exact string, and
`goalPriority`, `workGraph`, `planReconciler` and `planWedge` each re-derive it from the issue
number alone. An operator segment would have to be known at every one of those sites, and would
change the branch an in-flight issue resolves to — its open pull request would stop matching it.

The consequence to know: **the branch namespace on the remote is shared, so two fleets working one
repository would both cut `issue/<n>`.** What keeps that latent rather than live is the ownership
gate above — with `ownWorkOnly` on, each fleet picks up only issues _it_ tagged, so two of them
never reach for the same number. Turn it off on a repository another fleet is working and the
collision is on the remote, with nothing in the configuration to catch it.
...
input every layer below reads as _the provider reported no per-check detail_, whose right answer is to
