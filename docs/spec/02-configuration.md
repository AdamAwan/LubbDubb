...
| `environments`        | `list`          | `[]`                | Where landed work travels, and the command that answers whether a commit has got there — `{name, command}` per entry. Empty is the off switch. `fileOnly`: each entry is a shell command the harness runs. → [23](23-environments.md#configuring-an-environment) |     |
