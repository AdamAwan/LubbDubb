...
| `knowledgeColdDays`            | Assign, exactly as `knowledgeScopeStaleDays` does and for the same reason: `buildStateSnapshot` reads the running config by reference at every poll and takes the reading there, so the object _is_ the late reader. Live because it is a number an operator tunes while looking at the page it folds. → [27](27-knowledge.md#what-has-gone-cold)                                                                                              |
...
| `knowledgeColdDays`       | `number`                        | `30`                    | How many days a `proposal` nobody has agreed with and no agent has asked for may sit before the cockpit's Knowledge page stops **drawing** it — it goes behind a counted fold rather than out of the store. A **reading and never a trigger**, and narrower than that: it is defined only over `proposal`, the one reach that reaches nobody, so there is no prompt it can take a claim out of and no reach it can move. Derived from the rows the store already holds rather than recorded, for `knowledgeScopeStaleDays`' reason. `0` turns the reading off. Live: takes effect on the next poll, no restart. → [27](27-knowledge.md#what-has-gone-cold)                                                                                                                                                                                                                                                                                                                                                                           |
