...
| `localValidation.browser`             | `ExtraMcpServer \| null`                   | Playwright, headed, persistent profile | The MCP server that gives a validating agent a browser. A server rather than a flag because there is none inside a headless `claude -p`: Claude in Chrome and computer use both require an interactive session. `{outputDir}` and `{profileDir}` in its `args` are filled in per run. **`null` is a real configuration** — an API-only project validates through the API and the logs, and the prompt says so rather than letting the agent invent a screen. **Live.** → [32](32-local-validation.md#the-browser)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
