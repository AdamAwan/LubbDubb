...
| `validationRoot`     | `string`  | `.lubbdubb/validation`  | Root for a goal's validation resources — fixtures, reference material, sample data — one directory per goal (`<root>/issue-284/`). `attachmentRoot`'s storage rule, argument for argument: outside every worktree, canonical rather than copied per dispatch, and granted to every agent launch through `permissions.additionalDirectories`. Granted whether or not `validation.enabled`, so an agent's readable set does not depend on a policy flag it cannot see. → [20](20-validation.md) |
...
| `validation.enabled`                  | `boolean` | `true`                               | The validation plan: checks on the _delivered goal_, written by the planner and marked off by a person. **On by default**, and unlike the four above it spends no agent and gates nothing — the only consequence is that closing a goal with checks outstanding asks you to say why. Off leaves the surface out entirely: no checks are ingested, the plan sheet draws no section, and no goal is ever flagged. → [20](20-validation.md)                                                                                                                                                                                                                                                                                                                        |
