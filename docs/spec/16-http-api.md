...
reach without opening one would take the claim out of every prompt while putting it nowhere. The two
routes below are where that transition lives instead: one opens the pull request, and the sweep (or,
for a reading the harness will not take, `/settle`) moves the reach when it lands.
...
### `POST /api/knowledge/facts/:id/commit`

Commit a claim to the **repository** (#27 phase 6): open the documentation work for it, and record
that it is on its way. Body is a discriminated union on `target` — `{target: 'spec'}`, or
`{target: 'claudeMd', bar}` where `bar` is the operator's own statement of what breaks _silently_
without the claim. **404** when the fact is absent, **400** on an unknown target or a `claudeMd` with
no bar, and **409** on a claim the store will not commit — a proposal nobody has agreed with, a notice
that ends by itself, a claim already committed or otherwise terminal, or one whose documentation pull
request is still open. Broadcasts `world:changed`, runs a manual cycle so the job dispatches on this
click rather than the next heartbeat, and returns `{ok: true, fact, job, graduation, report}`.

**One call, because it is one act.** Opening the work and recording where it went are two halves of
"this belongs in the repository", and both half-landings are silent: a documentation pull request
nothing links to lands and takes the claim out of no prompt, and a graduation naming no job draws a row
on its way to a repository nothing is writing it into. `Store.commitFact` makes both writes in one
transaction and this route is the only way to reach it.

**The reach does not move.** The claim is still injected or still answered on lookup, and still open to
contradiction, until its pull request actually merges — a claim taken out of every prompt at the click
is one the fleet stops being told for a pull request that may be closed unmerged. The `claudeMd` arm
carrying its `bar` in the body's _shape_ is `narrowed`'s discipline above: that file is loaded into
every agent's context on every dispatch and its length is asserted rather than intended, so the arm
that could be taken by forgetting a field is the arm that gets taken.
...
been answered — by an earlier click or by the sweep. `landed` moves the fact to `committed` in the same
transaction; `abandoned` moves nothing at all. Broadcasts `dirty` and returns `{ok: true, graduation, fact}`.

This is the `committed` verb the reach route does not carry, and the objection that keeps it out of
...
**These five are the whole write surface the cockpit has on this store.** Nothing here files a claim
...
lessons store keeps. And nothing an agent can reach commits anything: a documentation pull request is a
dispatch a person promotes, for `report_finding`'s reason ([13](13-jobs-and-findings.md)).
...
| `knowledgeGraduations`          | Every attempt to put a claim in the repository, newest first, the abandoned ones included — the job, the target, the pull request the job produced, the outcome, and `reading`: what the sweep makes of that pull request right now (`waiting`, `landed`, `abandoned`, or `unknown` for one that left the world without ever being seen closed). The reading is `graduationReading`'s answer over the work graph, the same function the sweep settles on, taken server-side for `distinctCorroborators`' reason — a browser that worked out whether a pull request had landed would be a second implementation of the verdict that takes a claim out of every prompt ([27](27-knowledge.md#committing-to-the-repository)). |
