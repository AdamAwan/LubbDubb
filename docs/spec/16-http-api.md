...
The read also carries `handoff` — the part's `PrDescriptionHandoff`, or null.
→ [07](07-pull-requests.md#the-pull-requests-own-page-is-where-it-is-written)

### `POST /api/prs/:number/description/handoff`

Hands the part's description back to an agent: one `pr_description_handoffs` row, idempotent, which
rule `pr-describe` dispatches for. A `400` for a pull request that is not a part's, and for one the
operator has already described — their own version outranks the agent's.
→ [07](07-pull-requests.md#handing-it-back-to-the-agent)
