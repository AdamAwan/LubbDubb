...
So `SlotProcesses` (`src/worktree/slotProcesses.ts`) answers _what is holding this_, and the pool
sweeps at **both** ends of a slot's life — on `remove`, which is the release the agent reap and
`abandonUnstarted` both come through ([below](#release)), and again immediately before a hand-over's
wipe, because the release cannot cover a process started after it, or a slot whose previous occupant
outlived a release that never ran.
[Reclaiming an orphaned directory](#reclaiming-an-orphaned-directory) sweeps too, for the same hold
one layer down.

**It is asked twice, and the second time it is asked about a path.** A sweep before a wipe has no
path to name — nothing has been refused yet — so it asks about the slot. A wipe refused **after**
that sweep has named its own files, in git's `failed to remove <path>` lines, and the second probe
asks who holds **those**: `refusedPaths` parses them out of git's message, the probe answers for
them, what it names is swept, and the wipe is retried once before anything is concluded. The order
is therefore sweep, wipe, ask what the wipe refused, sweep that, wipe again — and only then
[condemn](#a-slot-that-cannot-be-emptied).
...
- **What counts as holding it is per-platform, because what _refuses the unlink_ is.** On Windows it
  is a mapped image or an open handle. With no path yet, the question is which processes have the
  slot as their executable path or name it on their command line — one pass over the cheap CIM
  table, which answers for **every** process on the machine however busy it is. With a path, the
  question is who holds that file, and the **Restart Manager** answers it out of the kernel's own
  handle table: `RmStartSession`, `RmRegisterResources`, `RmGetList`, one session per refused path.
  On POSIX nothing here holds an unlink at all (it removes a running image and a live process's cwd
  quite happily), so the sweep is for the other half of the release — a watcher that would go on
  writing into the tree the next occupant is about to be handed — and reads `/proc/<pid>/cwd` and
  `/proc/<pid>/exe` where `/proc` exists, the same walk whether or not a path was named, because it
  is already complete and already cheap.
- **Nothing asks every process what it has mapped.** A `EnumProcessModules` per process costs about
  100ms whatever it finds, and the access-denied ones are the slowest, so on an ordinary developer
  machine — four or five hundred live processes is a normal day — walking the table takes the better
  part of a minute. Bounding it does not help: a budget under the process timeout gets through the
  first third, and `complete: false` becomes the **steady state** rather than the exceptional reading
  the rest of this section is written for. The Restart Manager scales with the number of paths the
  wipe was refused on instead of with the number of processes on the machine, which is the shape of
  the question actually being asked. The holder the module walk existed to catch — a `node.exe` from
  outside the slot with a `.node` binding loaded out of the slot's own `node_modules` — is precisely
  a holder of a path the wipe is refused on, so it is **named** by the second question rather than
  hunted for by the first.
- **A release is therefore the weaker half, deliberately.** `remove` has no wipe behind it, so a
  holder only the refused-path question could name is not swept there. It is swept at the next
  hand-over, where the wipe names its file — which is the moment it actually costs anything.
- **A probe that could not answer is `null`, and never an empty list.** `holding` is three-valued:
  the processes, none, or _could not tell_ — a process table that refused, a `powershell` run killed
  at `TABLE_TIMEOUT_MS`, a `ps` that was not there, a Restart Manager session that would not start.
  Fold the third into the second and the slot is condemned reading "nothing is holding it", which
  points the operator somewhere else entirely. The warning that goes with it names the **reason** —
  killed at the timeout, the exit code, the shell's own first line of stderr — because `execFile`'s
  message is the whole script and no reason at all, and a timeout arrives with an empty stderr.
- **An incomplete reading is not automatically `null`.** What it found, it found: those holders are
  swept and the hand-over goes on. It is `null` only when it found **nothing**, because then what it
  could not ask is the only thing that could have answered and the reading was never taken. A reading
  is incomplete when more paths were refused than the probe asks about (`PATHS_ASKED`), or when the
  handle table declined one of them. Either way the operator is told, separately from the failure
  sentence, because a probe that could not ask everything is not a probe that failed. Output with no
  `complete` of its own — an empty pipe, a shell that died mid-report — reads as incomplete: the
  script always prints its report, so nothing to read is a report that was never finished.
- **A reading is a report to an operator, and nothing hangs off it any more.** It names whom to go
  and stop. Whether a slot can be handed out is answered by the wipe, not by the probe — see
  [below](#a-slot-that-cannot-be-emptied) — so a machine where the probe can never say loses
  sentences in the error log, never slots.
...
- **The fault names the processes, not the errno.** `slotUnusable` says which slot, what git refused,
  and which processes were **still** holding it after the sweep — asked, like the sweep before it,
  about the paths the wipe was refused on: the pid and the file each one has open. The operator's next move is to go and stop that process, and neither `Invalid argument` nor
...
- **It comes back on its own, by trying rather than by asking.** At `ensure`'s dead end, and nowhere
  else, for the reason the [salvage](#reclaiming-a-stranded-slot) runs there: it costs a wipe per
...
- **Revival performs the question instead of predicting the answer, and that is the whole point.**
  ...
  again **does** refuse again, right there, and stays out. It used to be asked of the process probe
  instead, revived only on a walk that got all the way through and found nothing. On an ordinary busy
  Windows machine that reading does not exist: every walk is cut short, a cut-short walk that found
  nothing is `null`, `null` is not an empty walk — so `revive` was **unreachable**, every condemned
  slot stayed condemned for the life of the process, the pool silently shrank over a long run, and a
  restart "fixed" it. Nothing was red, because each individual step was behaving as written. Any
  future gate on a reading the probe may be unable to take reintroduces exactly that.
