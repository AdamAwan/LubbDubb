...
`whole`. Those rows are the retired shape's other half: "one pull request" used to mean *zero parts*,
...
which is open items by construction, and a rule that could see a closed row would eventually act on
one.
