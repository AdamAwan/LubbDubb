...
| `reviewPacks.ts`      | `review_packs`, `review_marks`                                                  |
...
| `scratch_entries`                      | `scratch.ts`        | `decision` — the fork behind a pad entry as JSON (`PadDecision`), null on an ordinary note, which is what every row from before the witness log is and the right answer for all of them, so no backfill. Without the entry every database from before [31](31-review-packs.md#the-witness-log) has no column and every fork written there is a note that lost its decision in silence. `retrospectives` is declared empty for `validation_resources`' reason.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `review_packs`, `review_marks`         | `reviewPacks.ts`    | **None, declared empty**, on `validation_resources`' terms and for its reason — both are fresh `CREATE TABLE`s from [31](31-review-packs.md#where-it-lives)'s second stage, and the entry exists so the first column added to either is noticed there rather than read back as `undefined`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
`work_item_filings`, `work_item_ignores`, `pr_watch_seeds`, `pr_work_item_links`, `profile_overrides`, `local_runs`, `goal_landings`, `environment_reach`, `goal_arrivals`, `environment_gate_releases`, `environment_health`, `pr_review_externals`, `pr_thread_reopens`, `pr_replies_sent`, `pr_thread_labels`, `pr_archive`, `issue_bug_filings`, `review_packs`, `review_marks` and `local_validations` were all introduced as new tables and therefore needed no
...
| `scratch_entries`           | The shared scratchpad, one pad per issue (`issue:<n>`) and one per pull request (`pr:<n>`): what the agents working it left for whoever works it next, and for the retrospective. **Append-only** — no update and no delete exists above the table. `decision` holds the fork behind an entry as JSON, null on a note ([31](31-review-packs.md#the-witness-log)).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | keyed on `pad_ref` (`issue:<n>`); ties on `created_at` break on `rowid`, which is insertion order                                                                                                                                                                                                                                                |
...
| `review_packs`              | A review pack: one pull request's change restated as ideas, claims and anchors, as one JSON document — written whole, read whole, nothing queries inside it ([31](31-review-packs.md#where-it-lives)). `head_sha` is a column as well as a field in the document, because staleness is decided against the pull request's head on every load without opening the document. An older head's row is kept when a newer one is written; the newest written is the current one.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | `PRIMARY KEY (pr_number, head_sha)`; upserted, so asking again on one head replaces the pack                                                                                                                                                                                                                                                     |
| `review_marks`              | What a reviewer did to a pack — an attention override, an idea marked read — held beside the document and never written into it, so a pack rewritten against a new head keeps their marks. One row per hunk an idea owns, keyed to the code and never to an idea's id, which is minted per run. `read` and `attention` are two columns on one row, and each write names only its own.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | `PRIMARY KEY (pr_number, path, start_line, end_line)`; `head_sha` is recorded, not keyed                                                                                                                                                                                                                                                         |
