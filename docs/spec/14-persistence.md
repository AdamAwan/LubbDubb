...
| `knowledge_graduations`     | One attempt to put a claim in the **repository**: the documentation job an operator opened for it, the target they named (`spec`, or `claudeMd` with the `bar` sentence that arm costs), the pull request the job produced, and `outcome` — `landed`, `abandoned`, or null while it is still going (#27 phase 6). Its own table rather than columns on `knowledge_facts` because a fact can have more than one: a pull request closed unmerged leaves the claim exactly where it was and the operator free to try again, and columns would overwrite the record of the attempt that failed. Deliberately **not** a reach — the claim goes on being delivered while its pull request is in review, and `knowledge_facts.reach` moves to `committed` only when the sweep reads that pull request as merged, in the same transaction as the outcome. Brand-new, so no `ColumnMigrations` entry — which does not keep it exempt the next time it gains a column. → [27](27-knowledge.md#committing-to-the-repository)                                                                                                                                                                                                              | —                                                                                                                                            |
