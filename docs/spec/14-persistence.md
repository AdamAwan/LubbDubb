...
| `goal_arrivals`                        | `environments.ts`   | `watched_at` — when the post-deploy watch's window pass considered an arrival, whether or not it opened a window for it. The module declared **no** entries at all until this column, its tables all being fresh `CREATE TABLE`s — which is exactly the case the rule is about: a table being new _once_ does not keep it exempt. Without the entry the column is invisible on every database from before the watch shipped, so the freshness guard reads `undefined` for every arrival and either opens a window on the fleet's whole history or on nothing at all, with nothing red either way. It needs **no backfill**, and that is a property of the guard rather than an oversight — see below. → [29](29-post-deploy-watch.md#only-for-an-arrival-the-harness-watched)                                                                                                                                                                                                             |
