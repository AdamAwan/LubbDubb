...
| `pr_thread_labels`          | What the agent answering a review thread said it was about: `about_comment` (the reviewer's point was about a code comment rather than the code) and `changed_code` (the agent changed code rather than defending what was there), plus the thread's `path` and `author` read off the world baseline at record time ([18](18-observability.md#which-part-of-the-code-a-review-thread-was-about)). Written by `reply_to_review`, which is the one place a thread is answered one at a time — `report_remedy` files once per dispatch and cannot count threads. **The area is not stored**: the path is, and `areasForPath` derives the area when the reading is built, so a corrected rule corrects the whole back-catalogue rather than only the rows written after it. A reply with no thread writes **no row** — it is a comment on the pull request, anchored to nothing. | `(pr_number, thread_id)` is the key; upserted, so the fleet coming back to a thread replaces its label rather than counting it twice. How often it came back is `pr_replies_sent`, a row per reply |
