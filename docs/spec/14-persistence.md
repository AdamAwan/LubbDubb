...
| `knowledge_similarities`    | **Not yet built.** Which two claims a machine thinks are one claim — a pair, the score that matched them and when it was taken. A **suggestion table**: nothing here joins, promotes, merges or bars anything, and the operator's click on the cluster is what moves a claim. It exists as rows rather than as a read taken on the page for the reason every other count on that page is server-side — a similarity recomputed in the browser is free to disagree with the one an operator acted on — and because the pass that fills it is over the whole `proposal` set, which is the part of the store that grows. Written by _src/knowledge/cluster.ts_ and read by nothing in `src/dispatcher/`, as with every other table here. Brand-new, so no `ColumnMigrations` entry, which does not keep it exempt the next time it gains a column. → [27](27-knowledge.md#one-claim-written-two-ways)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | —                                                                                                                                                                                                                                                                                                                                                |
