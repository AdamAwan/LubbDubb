...
has since gained `ticket_ref` and then `where_at`/`detail`, `plans`/`plan_parts` have since gained the
fields above, and `local_runs` has since gained its six usage columns — which
...
| `lessons`                   | What working a goal taught about **working this repository** (#355) — kept in the store rather than in the tree, because the three properties that make it safe are ones a markdown pad cannot hold: the gate (`status`), the provenance (`origin_ref` + `created_at`) and the prune (`retired`). Read by the cockpit, and mirrored into `knowledge_facts` by `KnowledgeStore.adoptLessons` — a promoted lesson **is** an injected fleet claim, and since #27 phase 3 it reaches agents as one rather than through a block of its own. No dispatcher rule reads it at any status.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | —                                                                                                                                            |
| `knowledge_facts`           | What the fleet knows about **working this repository**, on three independent axes: `scope` (fleet, one check, one goal), `lifetime` (standing or expiring, with `expires_at`) and `reach` (proposal → lookup → injected → committed, or superseded, or rejected). `resolves_when` (#27 phase 4) holds the JSON condition that settles a notice before its clock — written only by the harness's own notice desk, and null on every other row, which is why it needed an `ALTER TABLE` and no backfill: null spells _nothing but the clock settles this_, and nothing before phase 4 could write one. `supersedes` names the fact an amendment sharpens, and is what exempts it from a rejected parent's bar — and what makes `superseded` (#27 phase 5) a reach of its own rather than a rejection: an amendment usually _contains_ the claim it replaces, so rejecting the original would bar the sharper claim's own words. Nothing in the dispatcher reads it, exactly as nothing reads `remedies`. `about_ref` and `where_at` (the unified intake) carry what a claim is _about_ and what locates it — never the observer's own origin, which `origin_ref` already holds — and are the third pair to need an `ALTER TABLE` with no backfill, for the plainest reason of the three: nothing before `raise` could name either, so null is the only true value on an older row rather than merely an acceptable one. A promoted `lessons` row is mirrored in as a fleet-scoped standing fact at reach `injected`, under an id derived from the lesson's — which is what makes that adoption idempotent across boots. → [27](27-knowledge.md) | —                                                                                                                                            |
