...
| `plans.ts`            | `plans`, `plan_parts`, `plan_atoms`, `plan_revisions`, `plan_amendments`        |
| `validation.ts`       | `validation_checks`, `validation_resources`                                     |
| `issueVerdicts.ts`    | `issue_conclusions`, `issue_deliveries`, `issue_shortfalls`, `issue_appraisals` |
...
| -------------------------------------- | ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `tasks`                                | `tasks.ts`          | `origin_title`, `origin_summary`, `dispatch_reason`, `rule`, `ci_checks`, `model` (the resolved `claude --model` value for the run, from the `agentModels` policy — null on every deployment that configures none), `effort`, `permission_mode` (the `claude --permission-mode` value for the run when the profile named one, overriding the fleet-wide `agentPermissionMode` — null when it did not), `profile` (the name that trio came from), `profile_source` (`pin` \| `rule` \| `default` — which level of the precedence chain answered, stored rather than re-derived because config moves under a finished run) Then `mcp_servers` — the MCP servers a dispatch carried beside the harness's own ([32](32-local-validation.md#the-browser)). On the row rather than re-derived at spawn, `model`'s reason: `AgentManager.resume` rebuilds a launch from it, and an agent re-attached without the server it was launched with comes back holding a conversation full of tool calls it can no longer make. Null on every dispatch that carried none, which is every one but a local validation.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `issue_runs`                           | `runs.ts`           | `dismiss_note` — what an operator said when they ended a run whose validation was not clear. Null for every row from before it existed, which is right: nobody was asked.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `validation_checks`                    | `validation.ts`     | `revision`, `amended_at`, `amend_note`, `actor`, `handback_note`, `claimed_by`, `claimed_at` — the band an amendment leaves on a check, and who is expected to run it. Declared empty when the table shipped on the argument that a table being new _once_ does not keep it exempt; the debt was collected one change later, and without these entries every older database would read `undefined` for all three and silently draw no band. `actor` and `handback_note` arrived one change later and fail more quietly still: a column whose absence reads as `human` is one whose absence is invisible, and the hand-over control would simply never take. `claimed_by` and `claimed_at` came with the desktop channel and are quieter again: absent, they read as "nothing is claimed" — true of every older database and true forever after, so the fleet keeps dispatching checks a person is in the middle of running. `steps` came with the test plan, and `capture` with the screenshot handback — the screen a `screenshot` step handed back, as a file **name** in the goal's validation directory; `proof` with the evidence a pass must hand back. Null on every row from before each existed, which is the right reading for all three and why none is backfilled. `area` and `expects` were declared here too and are **retired** — see [Retiring a column](#retiring-a-column). The two things that arrived with those changes and are **not** here are `state`'s new `captured` and `result_by`'s new `script`: a value an existing column may hold is not a schema change, and `checkStateOf` narrows anything unrecognised to `unrun`, so an old row lands on _nobody has got to it_ rather than on _there is an image here waiting for you_. `satisfies` came with the goal criteria's reading at delivery: null reads as _none_, which is true of every row before it, so nothing is backfilled. |
...
| `issue_appraisals`                     | `issueVerdicts.ts`  | `proposed_profile`, `profile_answered_at` — the profile an appraiser proposed for the work, and when the operator answered it; `proposed_parent`, `parent_settled_at`, `proposed_area_path`, `area_path_settled_at` — where on the backlog the appraiser said the goal belongs, and when the operator answered each. Null on every row from before they existed, which is the right reading for all four: no proposal, nothing asked.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| `plan_parts`                           | `plans.ts`          | `touches`, `atoms`, `rationale`, `acceptance`, `acceptance_met`, `size`, `expected_kind`, `outcome_kind`, `outcome_ref`, `outcome_summary`, `blocked_reason`, `blocked_by`, `profile`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `plan_atoms`                           | `plans.ts`          | `touches`, `acceptance`, `rejected`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `jobs`                                 | `jobs.ts`           | `origin_ref`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `job_schedules`                        | `schedules.ts`      | **None, declared empty.** Still a fresh `CREATE TABLE` with nothing added since. The entry exists so the next column added is noticed here rather than read back as `undefined`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| `local_runs`                           | `localRuns.ts`      | `cost_usd`, `input_tokens`, `output_tokens`, `cache_read_tokens`, `cache_creation_tokens`, `num_turns` — what the sessions holding the environment up have cost. Nullable, and null is **unmeasured**: a run from before these existed reported nothing, and every run of a PTY deployment reports nothing ever, so a zero would describe an unmeasured preview as a free one. No backfill for that reason. → [23](23-local-runs.md#what-it-costs) Then `commit_sha` — where the checkout stands, written by the start and rewritten by a refresh. Null on a row from before it, and no backfill because nothing refuses on it: the freshness reading says "could not compare". Named so because `COMMIT` is a keyword. → [23](23-local-runs.md#watching-the-environment)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `feature_summaries`                    | `tickets.ts`        | `headline` — added to a table that already existed everywhere, so `CREATE TABLE IF NOT EXISTS` would never have reached it. Its null needs no backfill ([above](#when-a-null-means-something)).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `goal_arrivals`                        | `environments.ts`   | `watched_at` and `sheeted_at` — when the post-deploy watch's window pass, and when remote validation's sheet pass, considered an arrival, whether or not it opened a window for it. The module declared **no** entries at all until this column, its tables all being fresh `CREATE TABLE`s — which is exactly the case the rule is about: a table being new _once_ does not keep it exempt. Without the entry the column is invisible on every database from before the watch shipped, so the freshness guard reads `undefined` for every arrival and either opens a window on the fleet's whole history or on nothing at all, with nothing red either way. It needs **no backfill**, and that is a property of the guard rather than an oversight — see below. `sheeted_at` is the same column one subsystem over, added to the same entry and reading the same guard. → [29](29-post-deploy-watch.md#only-for-an-arrival-the-harness-watched), [36](36-remote-validation.md#the-desk)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `goal_watches`                         | `watches.ts`        | `expect_under`, `expect_over`, `expect_baseline`, `unit`, `baseline_value`, `baseline_at` — a measure's expectation, what its query read before the work arrived, and when — plus `live` and `proposal`, which carry a declaration an agent made and nobody has ruled on yet. The module declared **no** entries until these, its three tables all being fresh `CREATE TABLE`s one change earlier: the same case again, one table over. Without them a threshold reads `undefined` on every database from before this build, which is a measure that can never fail, on exactly the deployments with a history. **No backfill**: `baseline_value` null means _never taken_, which the fold already reads as `unknown`, and the two `NOT NULL` columns carry defaults that are the honest reading of a row written before either existed — a signal declares no baseline, and a check the operator's plan approval already authorised is live. → [29](29-post-deploy-watch.md#persistence)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `watch_readings`                       | `watches.ts`        | `value` — a measure's number, null for a signal and for any observation that did not answer. Same change, same reason: without the entry every reading of a measure reads `undefined`, and a card that draws no number beside a verdict is a ruling with nothing behind it. → [29](29-post-deploy-watch.md#persistence)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `watch_windows`                        | `watches.ts`        | `extended_at` — when an operator last gave a window more time, and the module's third table to collect one. It is the column the table's own `settled_at` warning was written against, and it needs **no** backfill for a stated reason rather than by luck: null means _never extended_, which is true of every row written before it existed, and nothing in the running code reads that absence as a state. → [29](29-post-deploy-watch.md#closing)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `scratch_entries`                      | `scratch.ts`        | `decision` — the fork behind a pad entry as JSON (`PadDecision`), null on an ordinary note, which is what every row from before the column is and the right answer for all of them, so no backfill. Without the entry every database from before [forks](11-mcp-tools.md#forks-on-the-pad) has no column and every fork written there is a note that lost its decision in silence. `retrospectives` is declared empty for `validation_resources`' reason.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `pr_review_routes`                     | `prReviewRoutes.ts` | `skipped` — the triage's other answer, that a pull request needs no review at all (`review.allowSkip`). The module declared **no** entries until this column, both its tables being fresh `CREATE TABLE`s: the same case again, and without the entry the column is invisible on every database from before the option shipped, so `reviewSkipped` reads `undefined` for every route and no skip is ever honoured — the merge gate holding pull requests the fleet decided not to read. It needs **no backfill**, and that is a property of the value rather than luck: null is not a third state waiting to be resolved, it is exactly what every row written before this meant — routed to a mode and reviewed. Declared nullable in `SCHEMA` too, so the column has one shape on a fresh database and on a migrated one rather than two that read the same until something writes null. → [07](07-pull-requests.md#skipping-a-review-altogether)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `pr_reviews`                           | `prReviews.ts`      | `published_thread` — the provider's id for the thread the fleet's review published its findings into, written by the send rather than by the report ([07](07-pull-requests.md#what-the-publication-is-recorded-as)). The module declared **no** entries until this column, its table being a fresh `CREATE TABLE`: the same case again, and without the entry the column is invisible on every database from before it shipped, so every review reads as unpublished and no findings can ever read as dealt with. It needs **no backfill**, and that is a property of the value: null is exactly what every row written before this meant — nothing recorded a publication of this review — and the only other evidence is a thread's author, which is the inference `pr_replies_sent` exists to replace.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
`goal_watches.baseline_value` is the second, and it is one that needed **no** backfill for a reason
worth stating rather than assuming. Null there means _never taken_, which is a value the running code
acts on: a measure declaring `noWorseThan: "baseline"` reads `unknown` while it is null, because a
comparison against nothing is not a comparison that passed. What makes the column safe without a
backfill is that no database from before it can hold a measure at all — the document schema refused
one until the same change added it — so every null is a signal's, which never reads the column.
A backfilled zero would have been the wrong shape twice over: an invented before, and a comparison
every optimisation passes. → [29](29-post-deploy-watch.md#the-baseline-and-why-a-measure-is-not-trusted-without-one)

`watch_windows.settled_at` is the fifth. Null there spells _still watching_: the desk reads it to
decide what to settle and what to ask again, and a settled watch is never re-opened by a reading, for
the reason a confirmed landing is never re-asked. So a column added to `watch_windows` whose own null
means something, without a backfill gated on `ensureColumns`' report, reopens every settled window on
the boot an operator takes the build — every one of them asking its environment again, and every fixed
verdict back in play.

`extended_at` is that column, added since, and the warning is what it was read against rather than
something it got away with: its null means **never extended**, which is exactly true of every row
written before it existed, so there is nothing to backfill and nothing to gate. The test is not
whether a column is nullable but whether its absence is a _value the running code acts on_ —
`settled_at`'s is, and this one's is not. → [29](29-post-deploy-watch.md#closing)

`feature_summaries.headline` passes the same test, and passes it for a reason of its own: **the row
repairs itself**. Null there means _written before the field existed_, which the card draws nothing
for, and rule `feature-summary` rewrites a Feature's whole account the moment anything under it moves
— so a deployment fills in its own headlines as its work moves, without a pass. It is also the one
column here a backfill could not honestly write: what it holds is an agent's own sentence about where
a Feature is, and a migration inventing one would put the harness's words in the single field on the
...
considered for the first time opens a window only if its confirming reading is within two probe
...
the third option correct. `goal_arrivals.sheeted_at` is that argument word for word, for the sheet a
remote validation assembles — and it carries one rule of its own: an arrival on an environment that
declares no `validate` block is left **unstamped**, because stamping where the feature is off spends
the guard for the environment that turns it on next month.
→ [29](29-post-deploy-watch.md#only-for-an-arrival-the-harness-watched), [36](36-remote-validation.md#the-desk)
...
stating rather than assuming. Its null is read only as a _fallback_ behind `interrupted_at` — the
...
- **Every column is named in the copy.** `SELECT *` binds by position and would silently shift a row's
  columns along the day either shape gains a field. A copy may compute the names from `columnNames`
  on both tables rather than write them out, where the old shape varies by database — see [Retiring a
  column](#retiring-a-column).
- **`createTables` runs again after the old table is dropped.** An index keeps its name when its table
  is renamed, so the schema's `CREATE INDEX IF NOT EXISTS` finds it standing on the old table, skips,
  and the drop takes it — the new table would run the whole boot unindexed. The second pass is
  idempotent and puts every such index back.
...
`VALIDATION_REBUILDS` carries a second `validation_checks` entry, the retirement of `area` and
`expects` ([Retiring a column](#retiring-a-column)).
...
### A migration that must run once

A data migration that must fire exactly once is a `runOnce(id, run)`, gated on a `store_migrations`
row keyed by its id. The id is never edited in place: a changed id declares a second pass, which every
database that ran the first runs again on the next boot.

`run` returns how many rows it touched, kept for the record. Both a run that moved a thousand rows and
a run that found nothing are stamped, because the gate is about the run and not the rows — a
deployment with nothing to fold that was left unstamped would re-read the source tables on every boot
forever.

**A one-shot that writes into a table another gate reads has to be checked against that gate.** The
migration does exactly what it says and the gate fires exactly once, so neither errors — the damage is
in the _value_ the gate takes from rows it was never written to read. `spendRuledClaims` stamped ruled
claims into `pet_actions` as inert rows, and `beginVivarium`, taking `MIN(at)` over the whole table,
pinned the vivarium's start to the oldest ruling and paid the backlog after it out in one scan. The gate
now reads hatched rows only, which answers its own question whatever a later migration stamps.
→ [22](22-pets.md#the-vivariums-start)

#### There is none right now

The three that existed all folded rows into `knowledge_facts`, and they went with the store
([27](27-obstacles.md#what-the-claim-store-left-behind)) — `runOnce` itself with them, since `knip`
runs every rule at error and an exported function with no caller is a failure. The `store_migrations`
...
`CREATE TABLE` carries the full definition. `jobs`, `plans`, `plan_parts`, `agent_flags`,
`agent_files`, `issue_conclusions`, `issue_deliveries`, `issue_shortfalls`, `issue_appraisals`, `scratch_entries`, `retrospectives`, `issue_runs`, `priority_overrides`, `goal_priorities`, `work_nodes`,
`work_item_filings`, `work_item_ignores`, `pr_watch_seeds`, `pr_work_item_links`, `profile_overrides`, `local_runs`, `goal_landings`, `environment_reach`, `goal_arrivals`, `environment_gate_releases`, `environment_health`, `pr_review_externals`, `pr_thread_reopens`, `pr_replies_sent`, `pr_thread_labels`, `pr_archive`, `issue_bug_filings` and `local_validations` were all introduced as new tables and therefore needed no
migration entry **at the time** — but a table being new once is not a table staying exempt:
`plans`/`plan_parts` have since gained the fields above, `local_runs` has since gained its six
usage columns, and `goal_watches` and `watch_readings` gained theirs **one change after being
created** with `watch_windows` following one change later still — which
...
owned the table — `POOL_RETIRED_TABLES` in `pool.ts`, `REMOTE_VALIDATION_RETIRED_TABLES` in
`remoteValidation.ts` — and applied from `Store`'s constructor beside `renameTables`, before any module
is constructed. A table whose module went with it is declared where the list is applied instead:
`REVIEW_PACK_RETIRED_TABLES` in `store.ts` drops `review_packs`, `review_marks` and
`review_pack_shares`, whose rows were worthless once review packs were removed and nothing reads them.
The drop needs nothing recorded to say it has run: the table's existence is what says it is outstanding,
and the drop is what removes it.
...
`ensureColumns` only ever adds, so a column nothing reads any more leaves through a **rebuild** — the
same one-transaction path a key change takes — and two things have to happen in the same change,
because neither half is safe alone:

- **The column leaves the table's `ColumnMigrations` entry and the schema together.** `rebuildTables`
  runs before `ensureColumns`: a column dropped from the schema while still declared is added straight
  back by the next line, and the rebuild detects it again on the next boot — **the table rebuilt on
  every boot for ever**, and every rebuild a chance to lose whatever the copy forgot.
- **The rebuild is detected by the retired column's presence and copies every column the two shapes
  share.** `validation_checks.area` and `.expects` are the case: dead once every reader moved to the
  `suite` step ([36](36-remote-validation.md#how-a-check-comes-to-have-an-area)), and retired by the
  second `validation_checks` entry in `VALIDATION_REBUILDS`. A database carrying `area` may predate
  `steps`, `capture` and `proof`, which were added by `ensureColumns` one change at a time — so the
  copy is the intersection of `columnNames` on the old table and the new, not a written-out list: a
  hand list naming a column the old table lacks fails the boot, and one that forgets a column loses it
  silently. The missing ones land null, which is each column's documented reading for a row from before
  it. The entry also refuses a table still keyed on `plan_id`, which the first entry rebuilds and whose
  copy predates both columns.

Asserted in `test/validationColumnRetirement.test.ts` against a **file-backed** database, booted twice:
an in-memory store has no prior schema to disagree with and cannot show the repeat rebuild.
...
| `job_schedules`             | Recurring briefs: the prompt an operator wants queued on a cron schedule, and how far through that recurrence the harness has got. Intent, not work — a firing writes an ordinary `jobs` row, so nothing downstream of the queue knows a clock queued it. `next_run_at` is the whole of the scheduling state, recomputed from the clock at each firing rather than from the slot that fired.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | —                                                                                                                                                                                                                                                                                                                                                |
| `job_attachments`           | Images an operator attached to a brief (#249): what they are and where the file is. Bytes live on disk under `attachmentRoot`, never in the database.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | `UNIQUE (target_ref, idx)`                                                                                                                                                                                                                                                                                                                       |
...
| `usage_events`              | Timestamped per-report cost **deltas** (not cumulative) for the fleet's **agents**, so rolling windows are a `SUM`. `agent_id` is `NOT NULL` and is the join `buildRemedyInsights` prices a remedy's filing agent through — which is why a local run's delta goes in its own table rather than here.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | —                                                                                                                                                                                                                                                                                                                                                |
...
| `api_errors`                | One row per agent turn the model API refused (`API Error: …`) — agent, task, origin, model, kind (`safeguards`/`other`), the `Details` code and the message. Read only by `GET /api/api-errors`; **nothing gates on a row here**. → [18](18-observability.md#api-errors)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Never pruned                                                                                                                                                                                                                                                                                                                                     |
| `mcp_calls`                 | Every MCP tool call that reached a tool body, on either channel — the tool, the calling agent and the origin it carried, whether it was refused and with what words, how long the body took, and the arguments. The one table whose sole reader is a cockpit panel ([17](17-cockpit.md#mcp)), written because the failure it is about leaves no trace anywhere else: an agent whose `mcp__lubbdubb__*` grants were dropped is refused every call before it arrives, finishes on the sentinels and reports nothing. `agent_id`/`task_id` are null on a desktop call, which has no dispatch behind it, and on a fleet call whose credential could not be resolved — the second especially is worth keeping. `origin_ref` is copied at write time rather than joined at read time, so a task retargeted later does not silently re-file every call it ever made. **Nothing gates on a row here.** → [11](11-mcp-tools.md#what-is-recorded), [and below](#mcp-calls)                                                                                                                                                                                                                                                               | `args` is the only column ever cleared — see below                                                                                                                                                                                                                                                                                               |
| `surface_reach`             | What an operator looked at, and what they did there — the registry's subject and verb ([34](34-usage-metrics.md)), the cockpit place key it was seen at, when, and whether the place was reached by a link or by a direct address. Five columns and no more, and the shape is the privacy position rather than a summary of it: no ref, no title, no free text, and **no identity column**, which is refused rather than omitted — a fleet is an engineer, so the fleet id already carries whose behaviour a row describes, and the digest cannot withhold a field. `mcp_calls`' three properties in full: **nothing gates on a row here**, the write is called for its effect and its return discarded, and retention is ninety days dropped from the back. The cockpit is the only writer, in batches. → [34](34-usage-metrics.md#the-one-new-table)                                                                                                                                                                                                                                                                                                                                                                         | Rows are dropped whole after ninety days; no column is ever cleared                                                                                                                                                                                                                                                                              |
...
| `plan_atoms`                | The atoms of a plan — the pieces of it that could each have landed alone. `touches`, `depends_on` and `rejected` are JSON. Which part carries an atom lives on `plan_parts.atoms`, not here.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | `UNIQUE (plan_id, slug)`                                                                                                                                                                                                                                                                                                                         |
...
| `plan_amendments`           | A change somebody wants made to a plan that is already **running**, held until an operator answers it. One row per proposal: the amended `document` exactly as submitted, the `note` saying why, and the `author` (`agent` through `plan_correct`, `operator` through `plan_amend`). At most one is `pending` per plan — the gate is in `proposePlanAmendment`, not the schema, because the refusal has to quote the standing one. `status` ends `applied`, `declined` or `superseded`, the last being the terminal for a row the world overtook rather than a verdict anybody gave. Gates **nothing**: the plan keeps scheduling while the row is pending. → [08](08-planning.md#amending-a-running-plan)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | —                                                                                                                                                                                                                                                                                                                                                |
| `plan_caveat_answers`       | What an operator wrote beside a caveat's tick box when they approved a plan: the caveat id, its label kept alongside, and their words. Append-only, written at the accept and never on any other verdict; a caveat that was ticked and nothing more leaves no row. Gates **nothing** — the plan is released by the same accept that writes these — and nothing rewrites the plan's own prose with them, so a replanner still reads the planner's `reason` rather than the operator's answer. Read by `priorWorkBriefing` for the agents that work the plan, and by the plan sheet. → [08](08-planning.md#a-tick-can-carry-words)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | —                                                                                                                                                                                                                                                                                                                                                |
...
| `issue_appraisals`          | Whether an issue's goal text can be worked from at all, judged before anything is dispatched, and which model profile the appraiser proposed for its work. Gates the funnel twice over: on a refused goal, which expires when the text changes or the world moves, and on a profile proposal nobody has answered, which expires only on the answer or the text changing. It also carries the appraiser's **placement** proposals — the container the goal should hang off and the area node it should sit on — which gate nothing at all ([06](06-issue-pickup.md#where-the-goal-belongs-the-placement-proposals-issue-463)).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | `origin_ref` is `PRIMARY KEY`; `goal_ref` fingerprints the text judged; `proposed_profile` + `profile_answered_at` are the goal-profile gate; the four `*_parent` / `*_area_path` columns are the placement proposals, which gate nothing — a question's visibility is derived from the live work item, and only the operator's answer is stored |
| `scratch_entries`           | The shared scratchpad, one pad per issue (`issue:<n>`) and one per pull request (`pr:<n>`): what the agents working it left for whoever works it next, and for the retrospective. **Append-only** — no update and no delete exists above the table. `decision` holds the fork behind an entry as JSON, null on a note ([11](11-mcp-tools.md#forks-on-the-pad)).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | keyed on `pad_ref` (`issue:<n>`); ties on `created_at` break on `rowid`, which is insertion order                                                                                                                                                                                                                                                |
| `feature_summaries`         | One account per Feature of where it has got to, written by rule `feature-summary`'s desk agent and revised whenever something under the Feature moves. Gates nothing; the dispatcher reads `standing_key` and never a word of the prose.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | `origin_ref` is `PRIMARY KEY` (the container's own); upserted, so `created_at` dates the first account and a revision is one row                                                                                                                                                                                                                 |
...
| `local_run_cost_deltas`     | The same shape for **local runs**: one dated cost delta per usage report the sessions behind a run made. Two tables of deltas, added in exactly one place — `Store.sumUsageCostSince` and `listCostDeltasSince` compose them, and a third source of spend is added there or it is money nothing states. → [18](18-observability.md#two-tables-of-deltas-added-in-one-place)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | —                                                                                                                                                                                                                                                                                                                                                |
| `local_runs`                | The machine's one dev environment, one row per run: which goal's code is in it, the checkout, the pid, how it ended, and what its sessions cost — accumulated across up to two of them, because a teardown by a fresh session reports its own cumulative total from zero ([23](23-local-runs.md#what-it-costs)). Which statuses count as **live** (`starting`, `running`, `stopping`) is declared once in `LIVE` and the SQL derives its `IN` clause from it — three statements used to write the set out by hand with `LIVE` read by nobody, and a status added to some of them is silent both ways: a second run allowed beside a live one, or a row that stays live across every restart. The row **outlives the run**, because a start that failed is the case an operator hits and its reason has to be readable after the process is gone. At most one row is live, and `beginLocalRun` is what makes that true — it ends the last in the same transaction that writes the next, rather than trusting a caller to check first. → [23](23-local-runs.md#persistence)                                                                                                                                                      | `id` is `PRIMARY KEY`; `idx_local_runs_status`                                                                                                                                                                                                                                                                                                   |
...
| `goal_landings`             | The commit each of a goal's pull requests landed as, attributed to the goal it was work for. `on_integration` is whether the clone can place that commit on the integration branch: null means the desk has not asked yet, so the column needs no backfill and heals itself on the next pulse. → [24](24-environments.md#what-counts-as-a-landing). Keyed on the PR for `branch_reaps`' reason. Stored because it cannot be recovered: a squash merge leaves the branch with no ancestry link to its base, and the provider only offers the merge SHA while the PR is inside `closedPrWindowMs`. Written `OR IGNORE` — the merge is settled, and a second sighting in the window is the same fact arriving again. → [24](24-environments.md#recording-a-landing)                                                                                                                                                                                                                                                                                                                                                                                                                                                               | `pr_number` is `PRIMARY KEY`                                                                                                                                                                                                                                                                                                                     |
| `environment_reach`         | What an environment probe said about one landed commit. Written `OR REPLACE`, unlike a landing: a verdict is an observation of something that moves. Stored so the cost of the feature is not a process spawn per landing per environment per pulse. → [24](24-environments.md#the-desk)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | `(sha, environment)` is `PRIMARY KEY`                                                                                                                                                                                                                                                                                                            |
| `goal_arrivals`             | One goal's whole work confirmed in one environment, the first time it was. Stored because an arrival is a **moment** and reach is a status: without a row the ticket comment goes out every pulse and the signal reads as a state rather than as something that happened. `OR IGNORE` — a goal that grows another merge and is confirmed again has not arrived twice, and a replace would clear `announced_at` and comment again. `announced_at` is stamped once the arrival has been through the announce pass **whether or not it had anything to say**, which is what stops an environment that grows `arrival.comment` later from commenting on its whole history. Historical rows written by a fold that counted only landed work are discarded by `dropPartialGoalArrivals()` when a live code part is still owed; the fixed desk re-derives the real arrival once the whole goal is confirmed. → [24](24-environments.md#what-an-arrival-means)                                                                                                                                                                                                                                                                         | `(goal_ref, environment)` is `PRIMARY KEY`                                                                                                                                                                                                                                                                                                       |
| `environment_gate_releases` | The operator's "this goal is not waiting on an environment" — a docs change, a config change, work whose deployment nothing here can see. Lifts every gate on that goal at once. Cleared by deleting the row, so "not released" keeps one representation. `note` is `NOT NULL`: the row is the only account of why a goal was closed out with no environment ever confirming it. → [24](24-environments.md#lifting-the-hold)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | `goal_ref` is `PRIMARY KEY`                                                                                                                                                                                                                                                                                                                      |
| `environment_health`        | What one environment's own health check last said: the state, the tier, and the check's own reasons. One row per environment, replaced each reading — health is a status, not a history, and a row per interval would be a log nothing reads. `changed_at` survives a reading that says the same thing, so "red" can be drawn as "red since Tuesday"; it is moved by a change of state or tier and not by a change of reasons, since a shifting list under one tier is the same episode still running. → [24](24-environments.md#is-the-environment-well)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | `environment` is `PRIMARY KEY`                                                                                                                                                                                                                                                                                                                   |
...
| `pr_assign_asks`            | The operator's answer to "want to assign this to someone?", one row per pull request: `assigned` with the person, or `declined`. The row is what stops the ask coming back, and the `assigned` rows feed the shortlist. First answer wins (`ON CONFLICT DO NOTHING`). → [07](07-pull-requests.md#asking-who-should-look-at-it)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | `pr_number` is `PRIMARY KEY`                                                                                                                                                                                                                                                                                                                     |
| `pr_reviews`                | The fleet's own review of a pull request — verdict, summary, findings, the commit it read, and (`published_thread`) the thread its findings were published into, whose resolution is the one statement that they were dealt with. Written by the `review_report` tool and by nothing else; in particular never inferred from a comment on the provider, since a review the harness _published_ and a review it _took_ are different facts. **Keyed on the pull request, not on the commit**: the review runs once, so a key that moved with the diff would be invalidated by the first fix pushed after it, nothing would write another, and the merge gate would be unsatisfiable for the life of the pull request — unmergeable forever with nothing red. `head_sha` records what was in front of the reviewer and gates nothing. → [07](07-pull-requests.md#the-fleet-review)                                                                                                                                                                                                                                                                                                                                               | `pr_number` is `PRIMARY KEY`; upserted, so one pull request has one answer to "has this been reviewed"                                                                                                                                                                                                                                           |
| `pr_review_routes`          | How the harness decided to read a pull request: the triage's chosen mode and its reason, one row per pull request, written by the `review_route` tool. `skipped` is the triage's other answer — that the pull request needs no review at all (`review.allowSkip`) — and it is read by the merge gate as well as by the rule, so a pull request nothing will review is not one nothing can merge. **Separate from `pr_reviews`** rather than a column on it, because the merge gate is satisfied by a `pr_reviews` row existing — a row written early to carry a route would report the pull request as reviewed by the step that only decided how to review it. → [07](07-pull-requests.md#choosing-how-to-review)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | `pr_number` is `PRIMARY KEY`; upserted                                                                                                                                                                                                                                                                                                           |
| `pr_splits`                 | Whether a pull request wide enough to be worth a look holds one concept or several: rule `pr-split`'s verdict, its named concepts, the reason, and the file count the diff measured when it was read. Written by the `split_assess` tool, one row per pull request. **The row is the rule's memory**, and it is written for a `coherent` answer as much as for a `split` one — a pull request judged coherent and then pushed to must not buy a second reading, so the key is the pull request and never its head commit. Nothing gates on it: it holds no rule, satisfies no merge and produces no work; a `split` verdict draws a chip on the pull request and the split itself arrives, if it arrives, as a plan amendment an operator accepts. → [07](07-pull-requests.md#how-wide-a-pull-request-is)                                                                                                                                                                                                                                                                                                                                                                                                                      | `pr_number` is `PRIMARY KEY`; upserted                                                                                                                                                                                                                                                                                                           |
| `pr_thread_reopens`         | Review threads the operator has put back in front of the fleet ([07](07-pull-requests.md#reopening-a-thread)). A mark in the harness rather than a write to the provider: unresolving a thread on GitHub reopens the reviewer's question in _their_ inbox, where this says "come back to this" to the fleet — and for the common case, a thread nobody resolved that the harness merely answered last, the provider cannot express it at all. **Cleared by the harness's next reply into that thread** (`ActionExecutor`) and by nothing else: a mark that outlived the reply would hold the thread open against every later reading and dispatch for it every pulse, for the life of the pull request. Never folded into `world_baseline`, which stays the record of what the provider last said — that is what lets the ask be taken back.                                                                                                                                                                                                                                                                                                                                                                                   | `(pr_number, thread_id)` is the key; insert-or-ignore, so a second click does not re-stamp the time the operator asked                                                                                                                                                                                                                           |
| `pr_replies_sent`           | One row per review-thread reply the harness actually sent ([07](07-pull-requests.md#attribution-is-a-record-never-an-identity)), keyed on the provider's own id for the comment it created. Attribution is this record and never the reply's author: the credential the harness posts under is the operator's own on a single-operator deployment, so an identity test badged _their_ follow-up on _their_ review thread as the fleet's answer, flipped the thread to `answered`, folded that to `PrComment.handled` and dropped the comment before rule `pr-review-comment` saw it. Written at the one call site a reply leaves through, `sink.postPrReply` in `ActionExecutor`. A send the provider will not name writes **no row** — the thread keeps reading as work and the miss is recorded through `errors.record` — and there is deliberately **no backfill** for replies sent before the table existed, because the only evidence on those is the author.                                                                                                                                                                                                                                                             | `(pr_number, comment_ref)` is the key; insert-or-ignore, so a retried send is one reply and the time stays when the fleet answered                                                                                                                                                                                                               |
| `pr_thread_labels`          | What the agent answering a review thread said it was about: `about_comment` (the reviewer's point was about a code comment rather than the code) and `changed_code` (the agent changed code rather than defending what was there), plus the thread's `path`, `author` and `author_is_bot` read off the world baseline at record time ([18](18-observability.md#which-part-of-the-code-a-review-thread-was-about)). Written by `reply_to_review`, which is the one place a thread is answered one at a time — `report_remedy` files once per dispatch and cannot count threads. **The area is not stored**: the path is, and `areasForPath` derives the area when the reading is built, so a corrected rule corrects the whole back-catalogue rather than only the rows written after it. A reply with no thread writes **no row** — it is a comment on the pull request, anchored to nothing. `author_is_bot` folds the two sources that are facts at observation time — the provider's own word and the stamp `review.publishedThreadProperty` declares — and is **three-valued**: null is _neither said_, never _a person_. The third source, `review.machineAuthors`, is config and is applied at read time like the areas. | `(pr_number, thread_id)` is the key; upserted, so the fleet coming back to a thread replaces its label rather than counting it twice. How often it came back is `pr_replies_sent`, a row per reply                                                                                                                                               |
| `pr_archive`                | The last reading of every pull request the world has reported closed, kept for good ([the closed pull request archive](#the-closed-pull-request-archive)). `WorldSnapshot.closedPullRequests` is a window `closedPrWindowMs` wide, and the cockpit's goal page drew its closed rows off that list alone — so a goal's pull requests vanished from its page hours after they merged, and a goal delivered last month said nothing had ever named it. A row is a fact about the past and never a reading of the present: nothing re-fetches an archived pull request and no rule reads one. Written by `recordWorldChanges` from the window it has just recorded, which is where the landing sweep writes for the same reason — a hook on the merge itself loses every close that happens while the harness is down.                                                                                                                                                                                                                                                                                                                                                                                                             | `number` is the key; upserted, so the window re-reporting one merge every pulse refreshes the row rather than appending to it                                                                                                                                                                                                                    |
| `closed_pr_sweep`           | One row (`id = 1`): how far the closed pull request set has actually been swept, so a read can reach back past the window to the closes an outage hid ([how far the closed set has been swept](#how-far-the-closed-set-has-been-swept)). Written only after a read the provider answered, and only ever forward.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | `CHECK (id = 1)` — there is one sweep, not a log of them                                                                                                                                                                                                                                                                                         |
| `pr_review_externals`       | Pull requests a check **outside** the harness reported already reviewed (`review.reviewedElsewhere`), so the fleet neither reads the diff again nor holds the merge for a review that is already done. Its own table rather than a `pr_reviews` row, because that row means _the fleet read this and here is what it found_ — writing an external gate as one would put a verdict in the cockpit, the Decision log and the next agent's prompt that nothing in this harness performed. Only the "reviewed" verdict lands: a gate that has not passed yet may pass later, and a row for the absence of an answer would freeze it into one. → [07](07-pull-requests.md#a-review-that-happened-somewhere-else)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | `pr_number` is `PRIMARY KEY`; insert-or-ignore                                                                                                                                                                                                                                                                                                   |
...
| `goal_predictions`          | What an operator predicted about a goal before they first read its plan — four slots (`locus`, `cause`, `split`, `avoid`), free text, each individually skippable. Sent to one agent only, [the judge](#the-prediction-judge): [the prediction store is not on `Store`](#the-prediction-store-is-not-on-store).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | `origin_ref` is `UNIQUE`; one per goal, and not re-openable                                                                                                                                                                                                                                                                                      |
...
| `goal_criteria_alignments`  | The alignment check's reading of one criteria version against the ticket's own criteria: the verdict, each point's tag, and whether the operator pressed on past a `conflicting` one. [08](08-planning.md#the-alignment-check)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | `UNIQUE (origin_ref, version)`                                                                                                                                                                                                                                                                                                                   |
...
- **Keyed on `target_ref`, not on a job id.** What an attachment belongs to is the _goal_, not the row
  the request arrived as: a code brief with a tracker configured becomes a ticket, so its images
...
- **The ref is decided before anything is written.** A code brief with a tracker configured is
...
- **Nothing ages them out.** Attachments live as long as what they are attached to, so a plan written
  ...
  as. The **only** deletion is a brief cancelled before it ran, which nothing downstream can want;
...
`recordAgentNote(id, note)`, `sumUsageCostSince(iso)`, `listCostDeltasSince(iso)`,
`listUsageEventsSince(iso)`.
...
`addLocalRunUsage` adds instead, for the reason [23](23-local-runs.md#what-it-costs) gives. The three
window readers differ deliberately: `sumUsageCostSince` and `listCostDeltasSince` are both tables,
because "what has this deployment spent" has one answer; `listUsageEventsSince` is the agents' rows
alone, for the reader that joins them by id.
...
`mcp_calls` grows with every tool call the fleet makes — a couple of thousand a week on a busy
deployment — and the question it raises is what to do with the old ones. The answer is drawn on an
unusual line: **the rows are kept for ever and only the arguments are cleared**, after
`mcpArgsRetentionDays` ([02](02-configuration.md)).

That is deliberately not "compact old rows to aggregated numbers", which is the obvious shape and the
worse one here:

- **A row without its arguments is about eighty bytes.** What actually grows without bound is the
  arguments — a submitted plan document is tens of kilobytes — so clearing them removes essentially all
  of the growth while leaving every count exact.
- **An aggregate has to be summed at some grain, and that fixes today what a later reading may ask.**
  The Insights window offers `all`; a reading served partly from raw rows and partly from a rollup is
  the two-sources-for-one-window disagreement the shared window was introduced to remove
  ([18](18-observability.md#the-window)).
...
- **`0` records no arguments at all**, rather than recording and then clearing them. The setting is the
  off switch; the sweep is not.
- **Lowering it is retroactive.** A sweep at `0` clears every row that still carries arguments, with no
  date bound — not a cutoff of `now`, which would leave behind exactly the rows written in the same
  millisecond as the operator switching it off, for ever, while reporting that it had cleared them.
- **`args_dropped` is set as the text goes**, because a compacted call and a call that carried no
  arguments are different facts that would otherwise both read as `args IS NULL` — and the panel would
  report a fortnight of empty calls, believably.

It sweeps from the write path (rate-limited to hourly: a fleet making calls is the only one
accumulating arguments) **and** at boot, because an idle harness would otherwise hold arguments past
their window with nothing to trigger the sweep. A retention promise kept only while busy is not one.

### Issue verdicts, and the exclusion matrix

Four tables record a verdict about an issue, keyed on the same `issue:<n>` origin:
`issue_conclusions`, `issue_deliveries`, `issue_shortfalls`, `issue_appraisals`. Some pairs may coexist
...
`recordIssueConclusion`, `recordDelivery`, `recordShortfall`, `recordAppraisal` — keep their names,
...
`resolveIssueConclusion` ranks the two instead), and an appraisal answers a different question entirely.
...
- **A deliberate "clears nothing" is an explicit empty entry.** As four inline `DELETE`s, `appraisal`
...
**`obstacle_blocks` is deliberately not a fifth kind**, and the reason is the matrix's own. Every
table above answers _is this goal's work finished_ and clears the answers it contradicts; a block
answers _can it be attempted right now_, and its exit is the **obstacle** rather than the issue
([27](27-obstacles.md#blocked-is-an-answer)). Folded in, it would have to name what it clears — and a
block that cleared a delivery is delivered work handed back to the fleet, which is the failure
[20](20-validation.md#when-a-check-fails) names from the other side. It is owned by
`src/store/obstacles.ts` with the rest of the board, and nothing clears it but the ownership desk's
own sweep.
...
shortfall normalises `part_slug` against `cause`, an appraisal keeps `comment_ref` only while `goal_ref`
...
never edited or deleted. → [08](08-planning.md#a-tick-can-carry-words)
...
`status` is progress **for every value but one**. `retired` is a _declaration verdict_ — it records
that a document stopped declaring the slug — so a document that declares it again lifts it, back to
`pending` with `blocked_reason` and `blocked_by` cleared, since both explained a status which is going
away. The
...
`amendValidation(originRef, {checks, withdraw, resources, note})` — the same merge from one agent's
...
would only have to be terse to delete the validation plan it is failing. `listValidationChecks(originRef)`, `listAllValidationChecks`,
`listValidationResources(originRef)`, `listAllValidationResources`,
`getValidationCheck(originRef, checkId)` — one live check, shared by every writer that has to decide
...
`setValidationActor(originRef, checkId, actor)` — the operator's hand-over to the fleet, and its undo.
`claimValidationCheck(originRef, checkId, holder, staleBefore)` — the search, the decision and the write
in one synchronous method, so two sessions racing cannot both read "nothing is claimed".
`releaseValidationClaim(originRef, checkId)` — its undo, idempotent.
`recordValidationHandback(originRef, checkId, note)` — the fleet giving one back: `actor` to `human` and
the reason on the row, and **no reading**, because an agent that could not reach the environment
learned nothing about the goal.

`recordValidationResult(originRef, checkId, {state, note, by, until})` — one method for all five
...
Two changes landed with atoms ([08](08-planning.md#atoms--the-pieces-a-part-is-made-of)), and the
second is the one that is easy to get wrong.

`plan_atoms` is a new table, and it carries a `ColumnMigrations` entry in `PLAN_COLUMNS` from the day
it landed rather than later — a table being new is true **once**, and the next column added to it
would otherwise be invisible on every database from before.

`plan_parts.atoms` is a column on an **existing** table, so `CREATE TABLE IF NOT EXISTS` will never
add it: it is an additive `ALTER TABLE` in `PLAN_COLUMNS.plan_parts` beside `touches`. **Null means
"this plan predates atoms", and that is the correct answer for every existing row — no backfill is
owed.** This is the trap [When a null means something](#when-a-null-means-something) exists for, and
the reasoning runs the other way here: a backfill inventing one atom per part would put a declaration
in front of a reviewer that no planner ever wrote, and it would read as the planner's. A part with a
null `atoms` reads back as a part with no atoms, and every reader draws what it drew before.
...
needs an entry like any other. The same holds for `plan_amendments`, and it needs no backfill either —
the table is empty on every database from before it existed, which is the honest reading of "nobody has
proposed a change to a running plan here". Its `document` is kept serialized rather than exploded into
columns because it is re-validated at apply time by the same `validatePlanDocument` both transports
use: what an operator approved and what is ingested are then one document, and a document the schema
has since moved past is refused whole rather than applied in halves.
...
`feature_summaries` holds one row per Feature: the five fields an agent submitted saying where it has
got to ([17](17-cockpit.md#the-feature-summary)), and `standing_key` — the digest of where every child
stood when the row was written.

**The key is the whole of the re-write trigger.** Rule `feature-summary` compares it against the
standing now and dispatches exactly when the two differ, so an unmoved Feature costs one string
comparison a pulse and no agent, for ever. It is a hash rather than the standings themselves because
it is stored on every row and answers only ever "is this the same": a Feature with forty children
would otherwise put a few kilobytes of standing in the database per write.

It is **not** columns on `feature_colors`, though both are keyed on a container's number. A colour is
an assignment nothing may re-decide — its entire value is that it is the same tomorrow — and this is
rewritten every time the work moves. Two promises that different do not share a row.

The prose is upserted, so a revision is one row and `created_at` still dates the first time anybody
said where the Feature was. Nothing above the table reads a summary to decide anything: the rule is
handed origins and digests only, `retrospectives`' rule, and for its reason.
...
_permanently_ detached: its goal's spend reads as unattributed, an obstacle write-up watching for the
job's PR never settles, and the unrecorded-work detector offers a ticket for work a merged pull
...
### The closed pull request archive

`archiveClosedPrs(prs)`, `listArchivedPrs()` — `src/store/prArchive.ts`.

`WorldSnapshot.closedPullRequests` is a **window**: a provider reports a pull request for
`closedPrWindowMs` after it leaves the open set and then stops ([03](03-world-model.md)). That is the
right shape for the world — every rule that reads it is asking about _recently_ — and the wrong shape
for the cockpit's goal page, whose closed rows were drawn off it alone. A goal's pull requests
disappeared from its page a few hours after they merged, and the page of a goal delivered last month
said no pull request had ever named it. That page is where somebody goes to ask what the goal shipped.

So the pulse keeps them. `recordWorldChanges` writes every row of the window it has just recorded into
`pr_archive` as JSON, upserted on the number, and the cockpit is shipped the table beside the world as
`archivedPullRequests` ([17](17-cockpit.md#the-pull-requests-and-the-tail)). Four properties, each the
answer to the obvious alternative:

- **Written from the window, not from the merge.** A hook on the transition loses every close that
  lands while the harness is down; the window re-reports one for hours, so a sweep of it catches the
  close on whichever pulse is next — the same reasoning that makes a landing a sweep rather than a
  hook ([24](24-environments.md#recording-a-landing)). That only holds while the harness is up _during_
  those hours, which is what [the sweep mark](#how-far-the-closed-set-has-been-swept) supplies: a
  merge the harness was down for is read on the way back, rather than having aged out unseen.
- **Upserted on the number.** The window reports the same merge on every pulse it holds it, so an
  insert-only table would grow one row per pulse per pull request and the goal page would draw the
  same merge eight times. `first_seen_at` survives the replace; `closed_at` and the snapshot are
  refreshed.
- **The whole row, as JSON.** What is kept is a _reading_, not a set of columns somebody chose: the
  review threads on a merged pull request are where an unanswered review ends up, and a lean row would
  make the page claim the provider reports no threads.
- **Stale by construction, and never a fact about the present.** Nothing re-fetches an archived pull
  request, no dispatcher rule reads the list, and no verdict is folded for one — the same reason
  `world.closedPullRequests` carries none. A reader that wants to know whether something _merged_ asks
  the work graph, which is the durable record with a verdict behind it
  ([the work graph](#work-graph)); this table answers "what did this goal ship", and nothing else.

The archive never prunes. It grows by one row per pull request the fleet ever closes, which is the
size of the repository's history rather than of the fleet's activity, and the goal page's whole reason
for existing after delivery is that the oldest rows are still there.

### How far the closed set has been swept

`readClosedSweep()`, `recordClosedSweep(sweptTo)` — `src/store/prArchive.ts`, table `closed_pr_sweep`,
one row (`id = 1`).

A provider asks for the closes since `now - closedPrWindowMs`, which is a statement about the **clock**
where what every consumer needs is a statement about what has been **seen**. The two agree only while
the harness is up. Across a gap they do not: a pull request that merged three minutes after the last
pulse is already outside the window when the harness comes back, and because the window only moves
forward it is never asked for again. The retarget, the branch reap, the landing sweep and this archive
all lose that merge together, each silently — the archive most quietly of all, since it is written
_from_ the window and so holds no row either, which is why reading the archive instead of the window
is no remedy at all.

`swept_to` is how far the closed set has actually been swept, and `closedReadSince`
([15](15-integrations.md#the-closed-pull-request-read)) extends the next read back to meet it, capped by
`closedPrCatchUpMs`. Two properties, for the reasons `tracker_sweep` has the same two:

- **Only ever forward, and only after a read that answered.** The upsert takes the `MAX`, and the
  provider records the mark after the call returns rather than before it. A mark ahead of what was read
  skips those closes for good; a mark behind costs one wider read that writes the same rows again.
- **The mark is the read's clock, not the newest close in it.** Safe only because every read still
  reaches back a full window, so a close the provider dated late is caught by the next pulse. The mark
  is not a claim that nothing closed after it — it is a claim that the harness asked.

The table is new, so it needs no `ColumnMigrations` entry — but a table being new once does not keep it
exempt. A database from before it has no row, which reads as _nothing swept_ and asks the plain window:
the fix does not reach backwards, and a rung already stranded is still stranded.
...
`account_rate_limits` is one row, pinned to `id = 1`, and answers _how much is spent right now_ — the
...
`rate_limit_readings` is the same figures appended, keyed on `captured_at`, and answers _when it went_
...
`plan_mark_locus|cause|split|avoid`, with `plan_marked_at` for when it was answered.
...
The four slots are a **vocabulary**, and it changed once already: `hard` ("what will be hard?") and
`surprise` ("what would surprise you?") were retired for `split` ("how would I split this up?") and
`avoid` ("what should NOT happen?"). The two that went were the two whose mark could not be read off
the plan without an argument — what counts as hard is a judgement about the work rather than a claim
the document either bears out or does not, and a slot phrased as a thing you do _not_ expect leaves
`matched` running both ways at once. The two that replaced them are checkable against the document as
it stands: the plan's parts either fall the way the operator said or they do not, and the plan either
does the thing they said should not happen or it does not.

**A slot is a column, so retiring one is a rebuild and never an `ALTER TABLE`.** `ensureColumns` only
ever adds, so `goal_predictions` goes through `PREDICTION_REBUILDS` — keyed on `hard` still being
there — which recreates the table from `SCHEMA` and copies forward what still has a question to
answer. The columns it copies are the ones the table has carried since it was created, because a
database old enough to predate the mark columns carries `hard` too and has to survive the same pass.

**What carries over is the text of `locus` and `cause`, and nothing else.** Every mark is dropped, on
all four slots, so that [the stamps stay derived](#moment-one-and-why-a-mark-is-four-valued): a row
whose only mark was on a retired slot would otherwise keep a `plan_marked_at` saying a moment was
answered with nothing left in it to have answered, and the aggregate's count of answered goals would
become a count of goals somebody once opened.

**`goal_reveals` is not cleared, and clearing it would be the one unrecoverable mistake here.** A
reveal is the seal: `recordPrediction` refuses once one stands. Clear the reveals to re-offer the gate
on goals that lost their prediction and the record fills with predictions written by somebody who has
already read the plan, which is the one thing it cannot survive. A goal that met the old gate keeps
its reveal, loses the two retired answers, and is never asked again — the honest outcome, and the
reason a slot change is not free.
...
**Where the gate is on, the stamp is written when the
[intake sitting](08-planning.md#the-intake-sitting-stands-in-front-of-the-planner) closes, before the
planner is dispatched** — so
`criteriaStanding` needs no change: a version written in the sitting precedes the stamp and the plan
alike, and the labels keep their names although _reveal_ now means _the sitting ended_. A goal planned
before its sitting existed is still stamped at its plan's reveal.
...
- **Nothing is deleted on a status change.** Cancelled jobs, rejected claims, retired parts and
