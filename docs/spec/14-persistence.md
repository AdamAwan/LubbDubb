...
| `human_tasks` | `humanTasks.ts` | `kind` (`'ask'` default, so every row from before the close-out reads as one)                                  |
...
| `human_tasks`        | Work only a person can do. `part_id` is the only way one holds work off the fleet; nothing in the dispatcher reads this table. `kind` tells the harness's own close-out from everything a person or an agent asked for.                                                                                                                                                       | —                                                                                                                                         |
