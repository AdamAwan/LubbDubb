...
| `validation_checks`                         | `validation.ts`    | `revision`, `amended_at`, `amend_note` — the band an amendment leaves on a check. Declared empty when the table shipped on the argument that a table being new _once_ does not keep it exempt; the debt was collected one change later, and without these entries every older database would read `undefined` for all three and silently draw no band.                     |
