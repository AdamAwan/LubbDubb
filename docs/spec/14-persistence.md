...
| `validation_checks`, `validation_resources` | `validation.ts`    | **None, declared empty.** Both are fresh `CREATE TABLE`s and need no entry today. The entry exists so the next column added to either is noticed here rather than read back as `undefined` — a table being new _once_ does not keep it exempt. |
...
| `validation_checks`    | How anyone checks the _goal_ was met: one row per check on a plan. `id` is the author's merge key; `letter` is the handle a person types, assigned once and never reused; `uses` and `covers` are JSON arrays. The result is columns rather than a table — one current reading, `note_progress`'s argument. `check_do`/`check_expect` are named around DO being a SQLite keyword. → [20](20-validation.md) | `PRIMARY KEY (plan_id, id)`                                                                                                               |
| `validation_resources` | What those checks need that is not in the repository — fixtures, reference material, access. Named, never pathed; the file lives under `validationRoot`. `human_task_id` remembers the ask filed for an unprovided one, so a replan does not file it twice.                                                                                                                                                | `PRIMARY KEY (plan_id, name)`                                                                                                             |
...
`tasks(status)`, `jobs(status)`, `job_attachments(target_ref)`, `job_schedules(enabled, next_run_at)`, `findings(status)`, `plans(origin_ref)`, `plan_parts(plan_id)`, `validation_checks(plan_id)`,
...
`ingestValidation(planId, {checks, resources, supersededReason})` (merges on check id, assigns
letters, supersedes rather than deletes), `listValidationChecks(planId)`, `listAllValidationChecks`,
