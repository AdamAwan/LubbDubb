...
| `pr_reviews`                | The fleet's own review of a pull request — verdict, summary, findings, and the commit it read. Written by the `review_report` tool and by nothing else; in particular never inferred from a comment on the provider, since a review the harness _published_ and a review it _took_ are different facts. **Keyed on the pull request, not on the commit**: the review runs once, so a key that moved with the diff would be invalidated by the first fix pushed after it, nothing would write another, and the merge gate would be unsatisfiable for the life of the pull request — unmergeable forever with nothing red. `head_sha` records what was in front of the reviewer and gates nothing. → [07](07-pull-requests.md#the-fleet-review)                                                                                                                                                                                                                                                                                                                             | `pr_number` is `PRIMARY KEY`; upserted, so one pull request has one answer to "has this been reviewed"                                                                                                                                                                                                                                           |
