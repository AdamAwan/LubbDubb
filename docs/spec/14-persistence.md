...
| `knowledge_facts`                      | `knowledge.ts`     | `resolves_when`, `supersedes`, `superseded_by`, `about_ref`, `where_at` — the JSON condition that settles a notice before its clock, what fact an amendment sharpens, and what a claim is about and where it locates, added across multiple phases as the fleet's knowledge system evolved. `superseded_by` is the newest and needs no backfill: null spells _nothing stands in this claim's place_, which is what every row from before it already drew — a claim superseded by an amendment is found from the sharper side, through that amendment's own `supersedes`, and the page has never drawn the link the other way round. See [27](27-knowledge.md) for the reasoning behind each.                                                                                                                                                                                                                                |
...
`knowledge_graduations.exit` is the second, and the same shape read the other way. Every graduation
written before the exits merged was a documentation pull request, because that was the only kind there
was — so null there is not a weaker claim but a value **nothing recognises**: the sweep could not tell
whether to watch the work graph for the row, and the page would draw a claim that went to the
repository as one that went nowhere nameable. `stampGraduationsBeforeExits` writes `docs` onto them,
on the boot the column arrives and never again — run unconditionally it would rewrite the exit of
...
A rename changes nothing about the rows, so there is nothing to copy and nothing to resolve — and that
is exactly why it is easy to miss that it is a migration at all. `SCHEMA` creates tables by name, so on
a database from before the rename the new name is simply absent, and `CREATE TABLE IF NOT EXISTS`
stands an empty table up under it beside a full one under the old name that nothing reads any more.
Nothing errors. Every row that predates the rename is just gone from the harness's point of view.

`renameTables(db, renames)` (`src/store/migrate.ts`) is that path, and it follows the same rule as the
other two: the entries are a `TableRename[]` **declared by the module that owns the table**, applied by
the composition root.

- **It runs before the schema pass**, ahead of `rebuildTables` in `Store`'s constructor. After the
  schema has run there is nothing to rename onto — the empty new table already exists — so the whole
  migration silently becomes a no-op on exactly the databases that needed it.
- **It needs no `runOnce` id.** The old name is what says the rename is outstanding, and the rename is
  what removes it, so a second boot finds nothing to do without having to remember the first one ran.
  That is the one migration shape where the guard is free.
- **Both names existing is left alone**, not merged. Nothing can produce that state — a fresh database
  has only the new name, a migrated one has only the new name, an old one has only the old — so there
  is no honest merge, and the untouched old table is the evidence somebody would need.

One declaration exists. `ISSUE_VERDICT_RENAMES` in `issueVerdicts.ts` carries `issue_assays` to
`issue_appraisals`, after the goal assay was renamed the goal appraisal. Skipped, every held goal comes
unheld on the boot an operator takes the build, and every profile, parent and area-path question they
have already answered is asked again. Asserted in `test/appraisalTableRename.test.ts`, against a
database built under the old name.
...
### Repairing arrivals from the old reach denominator

`dropPartialGoalArrivals()` (`src/store/environments.ts`) is the sibling repair for a second stale
arrival shape. Before the reach fold counted outstanding plan parts, a planned goal could be recorded
as arrived when only its first merged part had been confirmed. The fold is fixed, but the
`goal_arrivals` row it already wrote still makes `openedGoals()` release a `validate` or `close_out`
hold and prevents the real arrival from ever being announced.

The row is **discarded**, not corrected. An arrival claims the goal's whole work is confirmed in an
environment, so promoting a partial confirmation into the final arrival would preserve the wrong
claim. The desk re-derives it from the repaired landings once every owed part is confirmed. The
composition root joins `plan_parts` to `plans` and passes the affected goal refs to the environment
module, which deletes an arrival when a non-abandoned plan still has a non-retired, non-settled part
whose expected kind is code or unstated. Retired, merged, concluded and non-code parts are not owed,
so their historical arrivals remain valid. The cross-domain query stays in the composition root so
each store module still owns only its own tables.

This and `repairPartRefGoals()` are instances of the same rule: when a change to what a fold counts
leaves rows the old count wrote, an arrival row is a claim rather than a label and must be discarded
rather than rewritten. Both repairs are unconditional and idempotent; the corrected fold cannot
write either stale shape again, and a second boot finds nothing to do.

### A migration that must run once

Every migration above is safe to run on every boot, and that is what makes them cheap: they are either
additive or they only ever fill nulls, so the second run finds nothing to do. A migration that
**copies rows** is not that shape, and it fails in two exactly opposite directions, both silent:

- Run on every boot, it re-creates rows an operator has since ruled on. A claim they dismissed comes
  back as a proposal; a claim they retired comes back beside its own retirement; the page fills from
  underneath with decisions that were already made. Nothing errors, and re-created work looks exactly
  like work arriving.
- Never run, it loses everything it was meant to carry, on the boot the operator takes the build. A
  knowledge page that is simply empty looks exactly like a deployment nobody has raised anything on
  yet.

`runOnce(db, id, at, run)` (`src/store/migrate.ts`) is the gate: a row in `store_migrations` keyed by
the migration's **own name**, checked, run and stamped in one transaction so a crash halfway leaves
the database exactly as it was. It is `pet_resets`' mechanism ([22](22-pets.md#clearing-the-vivarium))
lifted out of pets, because the hazard was never pets'.

**Keyed by name and never by "has any migration run"**, so a build can ship a second one. Which is the
other half of the rule, and the one that costs something to get wrong: **an id is never edited in
place**. Renaming one does not rename a migration — it declares a second one, which every database
that already ran the first then runs again. That is `VIVARIUM_RESET`'s rule and it is in CLAUDE.md for
the same reason: a constant has no test with an opinion about it.
...
#### The one that exists: folding the claim stores

`foldClaimStores` (`src/store/store.ts`, under the id `findings-and-lessons-into-knowledge-facts`)
carries every `findings` and every `lessons` row across into `knowledge_facts`, once. It lives in the
composition root rather than in a domain module because it reads two domains' tables and writes a
third's, which is the cross-domain join `test/storeModules.test.ts` refuses inside a module.

What each row becomes is the map in [27](27-knowledge.md#what-the-three-stores-became), and three of
its lines are load-bearing:

- **A dismissed finding becomes `rejected` and a retired lesson becomes `retired`.** The two words
  meant opposite things in the two stores, and folding the prune into the bar would refuse by name,
  forever, every claim an operator had merely tidied.
- **A promoted lesson is the fact it was already mirrored into.** `adoptLessons` had been writing one
  under an id derived from the lesson's since delivery moved, so the fold uses the same derivation and
...
**It copies and deletes nothing.** The `findings` and `lessons` rows stay where they are: a fold that
got a row wrong is recoverable while its source is on disk and is not once the source is dropped. Both
tables leave `SCHEMA` when the last reader of them does, so a fresh database will not have them at all
— which is why the fold asks `sqlite_master` before it reads either, and why it reads with `SELECT *`
rather than naming columns a database old enough may never have had.

Tests: `test/knowledgeFold.test.ts`.
...
`work_item_filings`, `work_item_ignores`, `pr_watch_seeds`, `pr_work_item_links`, `profile_overrides`, `local_runs`, `goal_landings`, `environment_reach`, `goal_arrivals`, `environment_gate_releases` and `issue_bug_filings` were all introduced as new tables and therefore needed no
migration entry **at the time** — but a table being new once is not a table staying exempt:
`knowledge_facts` has since gained four columns and `knowledge_graduations` two,
`plans`/`plan_parts` have since gained the fields above, and `local_runs` has since gained its six
usage columns — which
...
| `knowledge_facts`           | What the fleet knows about **working this repository**, on three independent axes: `scope` (fleet, one check, one goal), `lifetime` (standing or expiring, with `expires_at`) and `reach` (proposal → lookup → injected → graduated, or superseded, retired or rejected). `resolves_when` (#27 phase 4) holds the JSON condition that settles a notice before its clock — written only by the harness's own notice desk, and null on every other row, which is why it needed an `ALTER TABLE` and no backfill: null spells _nothing but the clock settles this_, and nothing before phase 4 could write one. `supersedes` names the fact an amendment sharpens, and is what exempts it from a rejected parent's bar — and what makes `superseded` (#27 phase 5) a reach of its own rather than a rejection: an amendment usually _contains_ the claim it replaces, so rejecting the original would bar the sharper claim's own words. Nothing in the dispatcher reads it, exactly as nothing reads `remedies`. `superseded_by` is the other end of it, and the only end a **merge** has: an amendment names the claim it sharpens, but four phrasings folded into whichever the operator kept leave nothing on the survivor naming the four, so both paths write it and _what replaced this_ has one answer rather than one that depends on how it was replaced. `about_ref` and `where_at` (the unified intake) carry what a claim is _about_ and what locates it — never the observer's own origin, which `origin_ref` already holds — and are the third pair to need an `ALTER TABLE` with no backfill, for the plainest reason of the three: nothing before `raise` could name either, so null is the only true value on an older row rather than merely an acceptable one. It is the **one** claim store: `findings` and `lessons` are rows here, carried across once by the fold above under ids derived from their own, which is what makes that copy idempotent by construction as well as by its gate. → [27](27-knowledge.md) | —                                                                                                                                                                                                                                                                                                                                                |
| `knowledge_corroborations`  | Who says a fact is true — one row per observation, carrying the observer's **own words**, never a counter on the fact. `goal_ref` is the _goal_ rather than the dispatch origin (`pr:412:ci` and `pr:412:comments` are one observation), and `session_id` is carried so an agent that inherited a conversation through a re-dispatch is not counted twice. → [27](27-knowledge.md#corroboration)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | —                                                                                                                                                                                                                                                                                                                                                |
| `knowledge_contradictions`  | Who says a fact is **wrong**, and what they say it should say instead — one row per contradiction, naming the amendment filed with it (#27 phase 5). Its own table rather than a stance column on `knowledge_corroborations`, whose shape it otherwise shares exactly: `distinctCorroborators` counts the rows of that table, so a discriminated row would be read as agreement by any reader that forgot the filter — a contradiction promoting the claim it disputes, with nothing red. `resolution` is the operator's answer (`amended`, `narrowed`, `dismissed`) and null while it is open; nothing here demotes, lapses or deletes the fact named. Brand-new, so no `ColumnMigrations` entry — which does not keep it exempt the next time it gains a column. → [27](27-knowledge.md#contradiction-and-why-it-does-not-delete)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | —                                                                                                                                                                                                                                                                                                                                                |
| `knowledge_asks`            | How often a claim was actually **wanted** — one row per fact answered to a `knowledge_ask` (#27 phase 7). Written only from the credentialed tool path (`AgentManager.askKnowledge`) and never from `KnowledgeStore.askFacts`, which the cockpit reads twice on every poll to project its delivery view: a counter inside that read would count the operator's own browser as fleet demand, growing fastest while nobody was looking at the page. What keeps the cockpit out is the observer columns rather than a filter somebody has to remember — there is no uncredentialed ask. A row rather than a counter for `knowledge_corroborations`' reason with one word changed: a corroboration carries words because the words are what an operator reads, and an ask has none, so what it carries instead is who and when. Counted as rows rather than voices and over the whole life of the claim, because the number carries nothing — it is read, and nothing is demoted for want of demand. Brand-new, so no `ColumnMigrations` entry, which does not keep it exempt the next time it gains a column. → [27](27-knowledge.md#what-it-costs)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | —                                                                                                                                                                                                                                                                                                                                                |
| `knowledge_similarities`    | Which two claims a machine thinks are one claim — a pair, the score that matched them and when it was taken. A **suggestion table**: nothing here joins, promotes, merges or bars anything, and the operator's click on the cluster is what moves a claim. It exists as rows rather than as a read taken on the page for the reason every other count on that page is server-side — a similarity recomputed in the browser is free to disagree with the one an operator acted on — and because the pass that fills it is over the whole `proposal` set, which is the part of the store that grows. Written by `src/knowledge/cluster.ts` and read by nothing in `src/dispatcher/`, as with every other table here. Brand-new, so no `ColumnMigrations` entry, which does not keep it exempt the next time it gains a column. → [27](27-knowledge.md#one-claim-written-two-ways)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | —                                                                                                                                                                                                                                                                                                                                                |
| `knowledge_graduations`     | One attempt to put a claim **somewhere other than in front of the fleet**: which `exit` it took (`docs`, `job`, `ticket`), the job an operator opened for it, the `target` document a `docs` exit named and the `bar` sentence a `claudeMd` one costs, the `pr_ref` or `ticket_ref` it produced, and its `outcome`. Its own table rather than columns on the fact, because a claim may be sent somewhere twice and columns would overwrite the attempt that failed — which is the one thing somebody deciding whether to try again needs to read. → [27](27-knowledge.md#sending-a-claim-on)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
for the same two verdicts; both are rows here, carried across once by the fold above.

One entry point files a claim _or_ records an observer as agreeing with the standing one, because from
the caller's side those are one act: an agent writes down what it learned, and whether anybody already
has is not something it can know. `claimsMatch` (`src/claims.ts`) decides which, inside one scope — and
it lives outside this module rather than in it because it is a rule about what one _claim_ is rather
than about this table, which is what let both stores share it while there were two.

`setFactReach` is **guarded in the write** rather than by a read-then-check, the discipline
`decideProposal` uses, and it refuses to move a rejected row at all — an un-reject would be a way to
lift the bar without reading the amendment that should have lifted it. Two corroborations from two
distinct goals carry a proposal to `lookup` and no further: nothing but an operator (or a fact with a
clock on it) reaches `injected`. → [27](27-knowledge.md)

`exitFact(fact, exit, work)` is the cross-domain write that sends a claim on: it creates the job and
records the graduation **in one transaction**, and lives on the composition root rather than in either
module because a store module may not reach a sibling's tables. `linkGraduationTicket(id, ticketRef)`
is the `ticket` exit's own landing — the item existing _is_ the exit being taken, so it stamps the ref
and moves the reach in one write. → [27](27-knowledge.md#sending-a-claim-on)
...
It is **not** a `knowledge_facts` row. A claim is an agent's testimony, attributed through a corroboration carrying its `agent_id`/`task_id` and
