...
`work_item_filings`, `work_item_ignores`, `pr_watch_seeds`, `pr_work_item_links`, `profile_overrides`, `local_runs`, `goal_landings`, `environment_reach`, `goal_arrivals`, `environment_gate_releases`, `environment_health`, `pr_review_externals`, `pr_thread_reopens`, `pr_replies_sent`, `pr_archive`, `issue_bug_filings`, `review_packs`, `review_marks` and `local_validations` were all introduced as new tables and therefore needed no
...
The four mark columns are columns on the version rather than a table of their own: a check answers one
version once, and the row it lands on is the row it read. The only `UPDATE` in `PrDescriptionStore`
writes them, addressed by id — never by "the current one", because the operator can edit while their
own Claude Code is still reading.
