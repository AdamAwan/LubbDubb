...
- **A read is the whole answer, or it is no answer.** Both providers page the list to its end — Azure by
  `$skip` in pages of 100 — because a read cut at one page still answers, still moves the mark, and puts
  every close past the first page out of reach for good. The same holds for the open list.
- **The author filter is the open list's.** `prAuthor` is compared with `sameIdentity`, which ignores case,
  on both lists; an exact match on the closed list alone drops every merge whose uniqueName the provider
  cased differently, while the open PRs keep showing.
...
field cannot be updated more than once in the same update` — so the two-op `remove`-then-`add`
  ...
  **write is verified**
  ...
  query, and the tag is still on the work item in Azure with nothing red anywhere.
