...
| `review_route`       | Choose which of the project's declared review modes a pull request gets — the triage's verdict, as a **name from an enum** rather than prose, because the reviewer's prompt, charter and model profile are all resolved from it before it runs. A name the project has not declared is refused. Refused to any caller whose own origin is not `pr:<n>:review-triage`. → [07](07-pull-requests.md#choosing-how-to-review)                                                                      |
...
- **Its point of use** — the dispatch prompt or the instruction block for the work it belongs to — for
  ...
  `appraise_issue`, `plan_not_needed`, `retro_submit`, `feature_summary`, `link_ticket`, the scratch
  pair, the validation pair. `plan_not_needed` sits here rather than beside `plan_submit` in the addendum because only a
