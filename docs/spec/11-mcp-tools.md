...
| Tool                        | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| --------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `raise`                     | The one door for anything in an agent's way that is not its goal, and anything true of the repository the repository does not say. Says what it hit and why it is not its own doing; the harness extracts the keys and answers with whether anybody else has hit it. **The call is the lookup.** → [27](27-obstacles.md#the-intake)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| `validation_plan`           | Declare the **whole** validation check set for a delivered goal, with a note saying where it departed from the plan's hint. Omission is withdrawal. Two dispatches may call it — the **assessor** that has just answered `delivered`, and the **validation planner** — declared once in `checkSetAuthoringIssue`, and it is refused from any other origin **and for a goal with no standing delivery**, so a set is never written against code that may still move. Declaring no checks is a complete answer and requires `emptyReason`. Refused outright, naming the flag, where `validation.checkSets` is off — it stays in the assessor's `tools/list` there, and a set no rule would put to anybody is worse than a refusal. → [20](20-validation.md#one-agent-two-outputs), [the gate](20-validation.md#the-authoring-gate)                                                                   |
...
| `remote_validation_listing` | Say where the runner's own selector listing landed, taken in the read-only checkout the run is carried out in and pinned to the deployed commit, before anything is invoked. The harness parses that file and answers with the selectors that survived — which are the ones the agent then invokes the runner with, and the only ones. **It has no field naming a selector and no count**: a path says where a file is, not what is in it, which is the whole of why a denominator may come through an agent at all. Refused to every caller but that run's own agent, by the parse of its origin, and advertised beside `remote_validation_report` in `RULE_TOOLS['remote-validation']`. → [36](36-remote-validation.md#the-report-tool)                                                                                                                                                          |
...
| `description_review`        | Report what a check of the operator's pull-request description against its diff found — `contradicted` and `gap` findings, the same argument `description_check` takes on the desktop channel, and never text. Recorded with `recordCheck` on the version id the prompt named. Refused to any caller whose origin is not `issue:<n>:describe-check:<pr>`, and for a version that is not that pull request's. → [07](07-pull-requests.md#every-description-is-checked-without-asking)                                                                                                                                                                                                                                                                                                                                                                                                               |
...
- **It exists because `PlanDocumentSchema` requires a part, and that refusal is right.** Work that is
  ...
  schema cannot express is _no work at all_. Without this tool a planner that reads the repository and
...
- **`detail` is required, where an assessment's is optional.** An assessor corroborates work the
  ...
  ticket with no run behind it, so the evidence _is_ the verdict. A bare "already done" is not
...
Arguments `{what, why_not_mine, fix_makes_it_go_away?, keys?, blocks_me?, until?}`. Two are required
and the rest are the harness's routing read off what the caller supplied. The full argument is
[27](27-obstacles.md#the-intake); what belongs here is what the channel contributes to it:

- **It takes no author, no goal and no destination.** Identity is structural, as it is for every write
  here (`token -> agent -> task -> origin`), and the goal a voice is counted under resolves from the
  same chain — an agent cannot report on somebody else's goal or corroborate itself. What is left for
  the caller is what it saw.
- **The routing is one boolean the agent can always answer.** `fix_makes_it_go_away` decides a column
  on the row and never which store it lands in; `until` is a backstop clock and `blocks_me` chooses
  the fourth directive. None of them is a word an agent can pick wrongly, which is the whole reason
  the tools they replaced could be collapsed at all.
- **The call is the lookup, and it answers in the response rather than only in the description.** It
  comes back with the row's state, its owner, what other goals saw and the near-matches nothing
  merged — in one round trip, with no model call. The report is filed either way and never held
  pending a reply.
- **It refuses a report of the agent's own breakage.** If the keys intersect the caller's own branch
  diff the call is refused, the file is named, and nothing is recorded — the only enforcement of _an
  agent fixes what its own session broke_ that is not a sentence in a prompt.
- **It queues nothing and parks nobody**, and says so in both places. An agent that raises something
...
[13](13-jobs-and-tickets.md#human-tasks) for the entity and its lifecycle. Four properties:
...
a story from the cockpit (see [13](13-jobs-and-tickets.md#filing-a-ticket)).
...
- **An unpushed head is diagnosed rather than quoted.** GitHub answers
  ...
  on — and the generic failure then advises opening the pull request _by hand_, which fails
...
<branch>` and _call again_, withholding the by-hand fallback that is right for an unwired tool and
...
`{body, about_comment, changed_code, thread?, resolved?}` — and, like every tool here, **nothing that
names a pull request**: it comes from the caller's origin.
...
- **`resolved` is how a thread gets closed, and it is the only way.** It rides on the same
  ...
  otherwise.
- **`about_comment` and `changed_code` are not part of the reply**, and no reviewer ever sees them.
  They are the fleet's own record of what the thread was: whether what the reviewer raised was about a
  **code comment** — one missing, stale, wrong, badly worded, or one that should not be there — rather
  than about the code, and whether the agent changed code for it or defended what was there. **Both
  are required.** Optional, they would be omitted far more often than they would be answered false,
  and an omission read as false is a number that deflates with nothing red; required, every reply
  carries a verdict. They are per thread and not per dispatch, which is the whole reason they ride
  here rather than on `report_remedy`: one dispatch answers every unhandled thread on a pull request
  and files **one** remedy, so a cause counts dispatches where this counts threads. The two readings
  are orthogonal — `report_remedy` says _why_ the fleet came back, this says _what the thread was
  about_. → [18](18-observability.md#which-part-of-the-code-a-review-thread-was-about)
- **A reply with no `thread` is labelled nowhere.** It is a comment on the pull request rather than on
  a review thread, so there is no thread for a row to be about, and inventing one would put a comment
  on nothing into whichever area came first.
...
- **It is named at its point of use**, in `pr-review-comment`'s appended block rather than in
  ...
  reads as an instruction to answer _somewhere_. → [05](05-dispatcher.md#prompt-templates)
...
The channel started narrower than this: reads and four steering verbs, with the inbox's decisions and
anything that put work in left to the cockpit. The operator asked for the rest, and the reason is the
whole point of the channel — **the cockpit is a browser tab on one machine**, and an inbox with rows
that can only be cleared there is an inbox that fills up while they are away. So the line moved, and
it now sits here:

**What this channel may do** is what the operator's own hands do in the cockpit: read everything,
steer the queue and the cap, settle every row in "Needs you", put a brief in, and drive a live agent.

**What it may not do** is the _fleet's_ surface. No tool here concludes a goal, writes a plan
document, opens a pull request, or reports a validation reading on work it did itself. Those stay
behind the origin an agent was dispatched on, because they are an agent's account of work it
performed and this session performed none of it. `validation_report` is the one apparent exception
and is not one: it reports a check this session **claimed and ran at the keyboard**, which is the
whole reason the channel exists.

**`job_create` is not a dispatch**, and the distinction is load-bearing rather than pedantic. A code
brief on a deployment with a tracker is **filed as a watched ticket** and enters the planning funnel
like any other issue — appraisal, plan, parts — so the harness still decides whether and when to work
it. That is `POST /api/jobs`' behaviour since issue #198, reached through the same `submitBrief`
(`src/jobs/brief.ts`) rather than a second account of it. The tool says so in what it hands back,
...
is also what clears the park, rather than writing to the session and leaving the agent answered and
...
Two extractions came out of that rule, and both were third-copy cases rather than tidying.
`submitBrief` (`src/jobs/brief.ts`) is the ticket-or-job transform a brief goes through, now shared
by `POST /api/jobs` and `job_create`: written twice, the copy without the watch label would file a
ticket that is created, linked, shown complete in the cockpit and **never dispatched for**.

The watch toggle is the other, and it cost the larger change. The cascade, the two label mirrors and
...
- **A dispatch may carry extra servers, and they ride the same document.** `Task.mcpServers` — the
  ...
  first time they added a tool, with the server still connected and only the new call refused.
...
The same rule decided the docs route the other way (#397). Something true of the repository is _not_
part of the retrospective's submission — any agent that learns one can file it, and the one that learns
it first is rarely the one writing the retrospective — so it went on the one intake rather than becoming
a second field on `retro_submit`, and that intake is `raise`. Which also means no new tool, no new grant
and no second cap: it reuses a row and a gate that already exist. `retro_submit` carries the document
alone, and its description says where the rest goes — a tool description always arrives, where an
operator's template override may not. → [27](27-obstacles.md#the-intake)
...
_runs_ that made no call rather than on the calls themselves. And on a run's **whole life**, never on
the slice of it a window holds: the launch either carried the grants or it did not, so a run that
called the channel at all did not have this problem, whenever it called.
→ [17](17-cockpit.md#mcp)
