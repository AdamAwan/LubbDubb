...
| `scratch_append`     | Leave a note on the shared scratchpad for the issue this agent is working. Append-only, attributed from the credential. Refused outside an issue subtree. **Not yet built:** an optional `decision` object beside the note marks the entry as a fork for [31](31-review-packs.md#the-witness-log), and a `pr:<n>:*` origin resolves to a pad of the pull request's own rather than being refused. |
