...
What it may _sum_ is narrower than what it carries: a slice the provider filtered to this operator is
this fleet's alone, and an unfiltered one — GitHub's issue sweep, or any fleet with `ownWorkOnly` off —
is the repository's, which every fleet watching it also reports.
→ [28](28-cross-fleet-pool.md#the-throughput-section), [15](15-integrations.md#what-a-snapshot-is-scoped-to)
