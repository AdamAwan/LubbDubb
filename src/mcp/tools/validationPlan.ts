...
import type { ValidationCheck, ValidationResourceInput } from '../../types.js';
...
export const validationPlan: ToolFactory = ({ deps, task, ok }) => ({
  ...
  handler: (args) => {
    ...
    return ok(authoredReply(written, resources));
  },
});

function authoredReply(written: ValidationCheck[], resources: ValidationResourceInput[]) {
  return {
    authored: true,
