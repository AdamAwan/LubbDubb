...
export const reportRemedy: ToolFactory = ({ deps, agent, task, ok }) => {
  ...
  return {
    description:
      ...
      'nothing.\n\n' +
      'When the guard is "undocumented" you may also propose a **lesson**: the thing about working ' +
      'this repository that, written down, would stop the next agent paying for it again. It lands as ' +
      'a proposal and reaches no agent until an operator vouches for it.\n\n' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        lesson: {
          type: 'string',
          description:
            'Optional, and only with guard "undocumented". One claim about working this repository ' +
            'that would have saved this round — a line or two. Omit it unless the fact is genuinely ' +
            'written down nowhere; if it is in the tree and you did not read it, that is guard ' +
            '"documented" and no lesson.',
        },
      },
    ...
    handler: (args) => {
      ...
      return ok({
        ...
        // Named rather than silent, for `retro_submit`'s reason: an agent that
        // believes it filed a lesson has no other way to find out it did not.
        lessonProposed: result.lessonProposed,
        note:
          ...
          (result.lessonProposed
            ? ' Your lesson is a proposal an operator rules on — it reaches no agent on your say-so.'
            : ''),
      });
    },
  };
