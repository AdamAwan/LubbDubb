...
export const MCP_PROTOCOL_ADDENDUM = [
  ...
  '  nobody: raise it and carry straight on.',
  ...
  '- knowledge_propose(claim, scope, evidence) when you learn something about working this repository that',
  '  the repository does not say, and would have wanted to read before you started. Call it when you learn',
  '  it rather than saving it for a write-up. It reaches no other agent on your say-so — and if somebody',
  '  has already filed the same claim, your call is recorded as corroboration rather than a second copy,',
  '  which is what carries a claim out of one agent’s head.',
  '- knowledge_notice(claim, scope, evidence, expiresInHours) when what you saw is true today and will stop',
  '  being true — a check that failed and then passed on the same commit, a registry refusing installs, a red',
  '  base branch. State what you SAW and never what to do about it: a second agent on a different goal',
  '  seeing the same thing puts it in front of every agent, and an instruction that reaches the fleet that',
  '  way is how a real defect gets waved through. It expires by itself, which is the only reason it may',
  '  travel that far.',
  '- knowledge_contradict(factId, amendment, evidence) when a claim the harness gave you is contradicted by',
  '  the repository in front of you. Say what it should say INSTEAD — a bare objection is refused, because a',
  '  claim that is right in general and wrong at one edge is worth sharpening rather than deleting. It moves',
  '  nothing on your say-so: the claim goes on reaching every agent until an operator rules, and you go on',
  '  working to what the code says.',
