import type { ConclusionAuthor, IssueConclusion, IssueConclusionVerdict, Plan } from './types.js';
...
/**
 * Fold an issue's stored declaration and its plan into one verdict.
 *
 * Precedence, first match wins:
 *
 * 1. **The operator's toggle.** Always wins, on any item — it is the escape hatch,
 *    and the only thing that can contradict a plan roll-up. An operator looking at
 *    a complete plan and saying "there is more to do here" must not be argued with
 *    by a derivation.
 * 2. **The agent's declaration**, from `conclude_work`.
 * 3. **The plan derivation**: `complete` → done, an in-flight plan → more work.
 ...
 *
 * Pure over its three arguments — no store, no world — so the one question the
 * dispatcher, the cockpit chip and the tool layer all ask has exactly one answer.
 */
export function resolveIssueConclusion(stored: IssueConclusion | null, plan: Plan | null): ResolvedConclusion {
  if (stored) {
