 ...
 *
 * Four rules dispatch a code agent whose prompt tells it not to commit or push
 * anything: `issue-appraisal` ("do not implement anything, do not open a pull request"),
 * `issue-assess` ("do not implement anything and do not open a pull request"),
 * `validate-check` ("this is not a branch to build on") and `validation-failed` ("you
 * are not fixing anything on this branch"). All four are cut from the default branch
 * for one reason each says out loud — the state they are asked about is *on* it. They
 * need a repository. None of them needs a branch.
 *
 ...
 * read, and `test/readOnlyCheckout.test.ts` asserts every one of them produces it.
 */
