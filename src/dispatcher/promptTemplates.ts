...
const REGISTRY: Record<PromptId, TemplateDef> = {
  ...
  'feature-summary': {
    ...
    template:
      ...
      'The reader is not in the fleet and does not read the tracker. They want four things, and each is a field on the `feature_summary` tool:\n\n' +
      '1. **Where this is** (`standing`, required) — two or three sentences. What works, what it is waiting on, and the one thing to take away. This is the whole of what the card shows before anything is expanded.\n' +
      '2. **What is usable** (`usable`) — what a person can see or do *today*, and on which environment. This is the question a status word can never answer, and the one worth most: name what they would look at to judge it themselves.\n' +
      '3. **What is blocking** (`blocked`) — what is stopping the rest, and what it needs from a person. Say which it is: an agent parked on an unanswered question is asking to be *answered*, and a goal that fell short twice is asking somebody to *decide*. They read the same on a board and they are not the same ask.\n' +
      '4. **What is left** (`remaining`) — item by item where that reads better than prose. Include the items **nobody is working**: an item the fleet cannot see is not queued behind anything, and a reader not told so will assume it is in hand.\n\n' +
      'Restate a verdict as what it means for the Feature rather than as it was said to another agent — an assessor writes for the next agent, and "three views cross-join the unscoped mask" is not what a person needs to hear. Quote and attribute where the sentence earns it.\n\n' +
