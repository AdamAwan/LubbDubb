...
const REGISTRY: Record<PromptId, TemplateDef> = {
  ...
  'issue-assess': {
    ...
    doc: 'Sent to a code agent for an issue that has had work and has nothing in flight (rule `issue-assess`). It reads the delivered state on the default branch plus the work graph via world_read, and casts a verdict with assess_issue. It runs in a read-only checkout rather than on a branch of its own (issue #396), so {branch} is the name its worktree is leased under and not a ref — it is still rendered for an override that predates that. Placeholders: {number} {title} {body} {branch}.',
  },
  'issue-assay': {
    placeholders: ['number', 'title', 'body', 'branch'],
    template:
