...
export function agentsOnOrigin(originRef: string | null | undefined, store: Store): Agent[] {
  if (!originRef) return [];
  return store.agents.listAgents().filter((a) => store.tasks.getTask(a.taskId)?.originRef === originRef);
}

export function retryResumeFor(originRef: string | null | undefined, store: Store): RetryResume | null {
  if (!originRef) return null;
  const onOrigin = agentsOnOrigin(originRef, store);
