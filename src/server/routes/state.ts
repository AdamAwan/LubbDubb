...
export function register(app: FastifyInstance, ctx: RouteContext): void {
  ...
  registerStateRoute(app, ctx);
  registerReadRoutes(app, ctx);
  registerConfigWriteRoutes(app, ctx);
  registerRestartRoute(app, ctx);
...
function registerReadRoutes(app: FastifyInstance, { system }: RouteContext): void {
  const { config, errors, liveConfig, updates } = system;
...
function registerConfigWriteRoutes(app: FastifyInstance, ctx: RouteContext): void {
  const { config } = ctx.system;
  const filePath = ctx.system.configFile;
...
function registerRestartRoute(app: FastifyInstance, { system, hub }: RouteContext): void {
  const { store, updates, agents, runtimeControl } = system;
...
function prepare(
  ...
  if (edits.text !== undefined) {
    ...
    for (const path of [...Object.keys(set), ...(edits.clear ?? [])]) {
      const refusal = fieldEditRefusal(path, set);
...
function fieldEditRefusal(path: string, set: Record<string, unknown>): string | null {
  const field = configField(path);
  if (!field) return `${path} is not a configurable field`;
  if (field.access === 'fileOnly') {
    return `${path} is edited in the file, not here`;
  }
  const env = envOverride(field);
  if (env) {
    return `${path} is set by ${env} in this harness's environment, which beats the file`;
  }
  return Object.hasOwn(set, path) ? fieldValueRefusal(field, set[path]) : null;
}

function commit({ system, hub }: RouteContext, text: string, next: Config): ConfigSavePayload | { failed: string } {
  const { errors, liveConfig } = system;
  const filePath = system.configFile;
  try {
