...
export function register(app: FastifyInstance, ctx: RouteContext): void {
  ...
  registerPlacement(app, ctx);
  registerSteering(app, ctx);
  registerVerdicts(app, ctx);
  registerDeliveryOutcome(app, ctx);
  registerBugFiling(app, ctx);
  registerGoalAgents(app, ctx);
}
...
function registerPlacement(app: FastifyInstance, { system, hub }: RouteContext): void {
  const { store, connector, harness, config, errors } = system;
...
function registerSteering(app: FastifyInstance, { system, hub }: RouteContext): void {
...
function registerVerdicts(app: FastifyInstance, { system, hub }: RouteContext): void {
  const { store, harness } = system;
...
function registerDeliveryOutcome(app: FastifyInstance, { system, hub }: RouteContext): void {
  const { store, harness } = system;
  app.post(
...
function registerBugFiling(app: FastifyInstance, { system, hub }: RouteContext): void {
...
function registerGoalAgents(app: FastifyInstance, { system }: RouteContext): void {
  const { store } = system;
