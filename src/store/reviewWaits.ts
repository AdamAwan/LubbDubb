...
export class ReviewWaitStore {
  ...
  foldReviewWaits(waiting: readonly number[]): void {
    ...
    const insert = this.ctx.prep(`INSERT OR IGNORE INTO pr_review_waits (pr_number, since) VALUES (?, ?)`);
    const existing = this.ctx.prep(`SELECT pr_number FROM pr_review_waits`).all() as { pr_number: number }[];
    const drop = this.ctx.prep(`DELETE FROM pr_review_waits WHERE pr_number = ?`);
  ...
  reviewWaits(): ReadonlyMap<number, string> {
    const rows = this.ctx.prep(`SELECT pr_number, since FROM pr_review_waits`).all() as {
      pr_number: number;
      since: string;
    }[];
