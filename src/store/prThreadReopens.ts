...
export class PrThreadReopenStore {
  ...
  setPrThreadReopened(prNumber: number, threadId: string, reopened: boolean): void {
    if (!reopened) {
      this.ctx.prep(`DELETE FROM pr_thread_reopens WHERE pr_number=? AND thread_id=?`).run(prNumber, threadId);
      return;
    }
    this.ctx
      .prep(`INSERT OR IGNORE INTO pr_thread_reopens (pr_number, thread_id, reopened_at) VALUES (?,?,?)`)
      .run(prNumber, threadId, this.ctx.now());
  }

  prThreadReopens(): PrThreadReopen[] {
    const rows = this.ctx
      .prep(`SELECT pr_number, thread_id, reopened_at FROM pr_thread_reopens ORDER BY reopened_at`)
