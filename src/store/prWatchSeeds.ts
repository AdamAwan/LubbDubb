...
export class PrWatchSeedStore {
  ...
  recordPrWatchSeed(prNumber: number, branch: string): void {
    this.ctx
      .prep(`INSERT OR REPLACE INTO pr_watch_seeds (pr_number, branch, at) VALUES (?, ?, ?)`)
      .run(prNumber, branch, this.ctx.now());
  }

  seededPrs(): ReadonlySet<number> {
    const rows = this.ctx.prep(`SELECT pr_number FROM pr_watch_seeds`).all() as { pr_number: number }[];
