...
export class RemedyStore {
  ...
  recordRemedy(input: RemedyInput): Remedy {
    ...
    if (existing) {
      this.ctx.prep(`UPDATE remedies SET updated_at=? WHERE id=?`).run(ts, existing.id);
  ...
  listRemediesSince(since: string): Remedy[] {
    const rows = this.ctx
      .prep(`SELECT * FROM remedies WHERE created_at >= ? ORDER BY created_at ASC, rowid ASC`)
  ...
  listRecentRemedies(kind: RemedyKind, limit: number): Remedy[] {
    const rows = this.ctx
      .prep(`SELECT * FROM remedies WHERE kind=? ORDER BY created_at DESC, rowid DESC LIMIT ?`)
