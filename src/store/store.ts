...
export class Store {
  ...
  private migrate(): void {
    ...
    this.adoptFloorCompletions();
  }

  /**
   * Carry `floor_completions` (#203) into `issue_runs` (#234), then drop it.
   *
   * A reshape rather than an `ALTER`: `completed_at` was `NOT NULL` and a run
   * minted at pickup has no completion, so stretching the column to mean two
   * things would leave "minted" and "finished" indistinguishable on exactly the
   ...
   *
   * Guarded on the new table being **empty**, not on the old one existing: a
   * second boot has nothing to carry, and re-running the copy would overwrite
   * refreshed snapshots with the stale titles of the old shape. Carrying
   * `dismissed_at` is the load-bearing part — a live database holds dismissals the
   * operator has already made, and a backfill that silently dropped them would put
   * every cleared card back on the floor, now with the dispatcher acting on it
   * again. In one transaction, so a failure leaves the old table in place to
   * retry from rather than half a history.
   */
  private adoptFloorCompletions(): void {
    const old = this.db
      .prepare(`SELECT name FROM sqlite_master WHERE type='table' AND name='floor_completions'`)
      .get() as { name: string } | undefined;
    if (!old) return;
    const { n } = this.db.prepare(`SELECT COUNT(*) AS n FROM issue_runs`).get() as { n: number };
    this.db.transaction(() => {
      if (n === 0) {
        this.db.exec(
      ...
      this.db.exec(`DROP TABLE floor_completions`);
    })();
  }
   ...
   * whole point: a retained run is dispatched from, so its body and labels must
   * be the ones the issue last actually had.
   *
   ...
   * no-op rather than a re-stamp, and it returns whether it changed a row. This
   * now stops the dispatcher as well as the card — see
   * `docs/spec/17-cockpit.md` — so an accidental dismissal is undone by the goal
   * being worked again, not by an un-dismiss.
   */
  ...
  /**
   * Every run, dismissed ones included — the dispatcher needs the dismissal to
   * stop acting on a goal, and the cockpit needs it to tell a live goal from a
   * cleared one. Unbounded in age for `listWorkItemIgnores`' reason: a retention
   * that aged out of a window would put a goal back in front of the operator (and
   * back in front of the dispatcher) after they had ended it.
   */
  listIssueRuns(): IssueRun[] {
    const rows = this.db.prepare(`SELECT * FROM issue_runs ORDER BY started_at DESC`).all() as IssueRunRow[];
