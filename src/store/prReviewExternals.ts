...
export class PrReviewExternalStore {
  ...
  recordPrReviewedElsewhere(prNumber: number, detail: string): void {
    this.ctx
      .prep(`INSERT OR IGNORE INTO pr_review_externals (pr_number, detail, at) VALUES (?, ?, ?)`)
      .run(prNumber, detail, this.ctx.now());
  }

  prsReviewedElsewhere(): ReadonlySet<number> {
    const rows = this.ctx.prep(`SELECT pr_number FROM pr_review_externals`).all() as { pr_number: number }[];
