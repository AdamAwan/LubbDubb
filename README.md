...
- **`planning`** — the **planning funnel** for multi-PR issues. **Off by default** (`planning.enabled: false`), and off leaves it out entirely: no planner is ever dispatched and issue pickup behaves exactly as it does without plans. Turn it on and every watched, open issue gets a **planning agent first** (rule 3c, on its own branch `plan/issue/<n>`): it reads the repository and writes a verdict to `.lubbdubb/plan.json` in its worktree — `"single"` (one PR will do) or `"parts"` (a decomposition into stacked PRs, each with a `slug`, `scope` and `dependsOn`). The verdict is **persisted for both outcomes**, so a planner never re-runs on the same issue, and rule 4 then fires only for an issue whose plan says `single` — its behaviour for such an issue is otherwise unchanged, which is what turns today's one-agent/one-PR path into an explicit outcome of the funnel rather than a bypass. Two things worth predicting before you enable it: **planners outrank pickups** for scarce headroom (a planner unblocks work, so it should win a slot ahead of the work it unblocks), and a planner that crashes or writes no plan **fails open** — once the usual attempt cap is spent the issue routes to `single` and is worked the way it is today, so a planner failure can never park an issue. `planning.maxConcurrentPartsPerIssue` (default 2) caps how many parts of one plan may have agents at once. The graph lives only in LubbDubb's store (`.lubbdubb/` is gitignored, so `plan.json` is a side channel, not a committed artefact); only the `"rule"` dispatcher implements the funnel. The prompt is operator-overridable as the `issue-plan` template.
