...
| `Connector`         | The seam to the outside world. Behind it, the world is assembled from modular per-capability **integrations** (source control, issues), each with an interchangeable provider chosen in config; `CompositeConnector` merges their slices. v1 ships a `fake` provider per capability (an editable, persisted world you inject events into). |
...
| `EscalationInbox`   | The human-in-the-loop surface; routes answers into live agents or the next cycle, and auto-dismisses an agent's open escalations when it dies for good (kill/crash, or a recovery you settle) so "Needs you" never lingers un-actionable.                                                                                                  |
...
| `RecoveryDesk`      | Where agents orphaned by a crash or a shutdown wait for your restore / requeue / remove. Holds the heartbeat until every one is decided, so nothing new is queued in front of work already in flight.                                                                                                                                      |
...
- **`agentPermissionMode`** — passed to `claude --permission-mode` (default `acceptEdits`). `acceptEdits` auto-accepts **file edits only** — so on its own a headless agent still hangs the moment it runs `npm run check`, `git` or `gh`, with no human at the prompt. `agentAllowedTools` and the permission backstop (below) are what close that; you should not need `bypassPermissions`, which maps to `--dangerously-skip-permissions` (removes every gate at once, refused under root).
- **`agentAllowedTools`** — tool allow rules pre-approving the mechanical validate/commit/push commands so the default config takes an issue through to an opened PR **unattended** — without handing every agent unrestricted shell access. They're merged into `--settings` as `permissions.allow` (Claude Code syntax, e.g. `"Bash(npm:*)"`), deliberately **not** onto `--allowedTools` (which carries the MCP tool grants — mixing a Bash rule in risks silently dropping them). Default: the JS toolchain (`npm`/`npx`/`pnpm`/`yarn`/`node`), `git` and `gh`. Anything **not** listed doesn't get unrestricted access — it's routed to you by the permission backstop instead of hanging.
- **`mcp.permissionEscalation`** — the permission backstop (default on, gated by `mcp.enabled`). A tool call `agentAllowedTools` doesn't cover is routed to you via Claude Code's `--permission-prompt-tool`: it appears in **Needs you** with the exact command, and the **same** agent blocks until you click **Allow** or **Deny** — no config-and-restart, and a denial doesn't lose the agent. Since the allow-list covers the mechanical happy path, this only fires for genuinely unusual commands; a blocked agent holds its concurrency slot until you answer (or kill it). Turn it off to fall back to Claude's default headless deny.
...
- **`labelPrefix`** — the prefix behind the cockpit's **watch / ignore** toggle, shared by PRs and issues (default `"lubbdubb"`). It derives two labels — `${labelPrefix}-watch` ("work this") and `${labelPrefix}-ignore` ("leave this alone") — read by the dispatcher gates and written by the toggle. Precedence: an explicit `-ignore` always wins, then `-watch`, else the **type default**, which differs by kind: **PRs are opt-out** (worked unless tagged `-ignore` — the historical `lubbdubb-ignore` behaviour) and **issues are opt-in** (left alone unless tagged `-watch`). Every open issue/PR stays fully visible in the cockpit and `/api/state` (with its health / pickup verdict, so you see _why_ it's untouched) — the gate only decides what's _acted on_, provider-agnostically (`fake`/`github`/`azure`). Toggle an item from the cockpit's per-row **watch / ignore** button (which writes the labels through the provider) or apply the labels directly in GitHub/Azure. Set `labelPrefix` to `""` to disable both gates entirely (PRs never excluded, all open issues worked).
...
- **`assessment`** — the **assessor** (rule 3e). **On by default.** An issue that has had agents on it, has nothing in flight and no open PR gets one code agent to read what was actually delivered and say whether the goal is met. A `delivered` verdict is the harness's own park: weaker than the tracker's `closed`, reversible, and it stops pickup and nothing else. It is what keeps rule 4 from re-picking work whose PR merged and left the open list — which on GitHub nothing else prevents. Off, no assessor is dispatched and no issue is ever parked as delivered.
- **`assay`** — the **goal assay** (rule 3f). **On by default.** Before anything is started for a watched issue, one code agent reads the ticket against the repository and says whether there is a goal here to work from. It is the only gate in front of an issue that asks about _content_ rather than policy. Only an explicit `unclear` holds anything, it leaves one comment on the ticket saying what it would need, and the hold ends by itself the moment the ticket's text changes or anyone comments. A crashed or capped assayer writes nothing and holds nothing.
- **`retrospective`** — the **write-up** (rule 3h). **On by default.** Once a goal is parked as delivered and nothing is in flight under it, one **desk** agent writes the run up: what shipped, and what came out of the process of shipping it. It is handed the issue's shared **scratchpad** (what the working agents left each other through `scratch_append`) and the record only the harness has — which rules fired, what was escalated and how it was answered, replans, shortfalls, agents spawned and spend. The document is stored against the issue and read in the cockpit on the Goal Floor's Manifest station; **nothing is posted to the tracker and nothing is scheduled from it**. It gates nothing, so a writer that crashes or spends its attempt cap costs the report and nothing else.

  **The cost of all four, stated.** With `planning`, `assessment`, `assay` and `retrospective` on — the defaults — one issue can spend an assayer, a planner, its part agents, an assessor and a writer. Each is one agent, each is bounded by the same attempt cap, and each fails open; but if you want the harness to do nothing but write code, these are the four switches to turn off.
...
### Crash recovery (your call, before anything else runs)

Agents are child processes of the server, so restarting it — a crash _or_ a graceful `SIGINT`/`SIGTERM` — kills every agent mid-task. What happens to that work is **your decision**, and the harness waits for it.

- On boot the harness finds every agent the last run left in flight and parks it. **Until each one is decided the heartbeat is held**: no dispatch, no merges, no plan reconciliation, nothing queued in front of work that was already running. The cockpit shows them as a banner above everything else (the heartbeat chip reads `pulse held`), and the startup banner lists them in the terminal.
- Three choices per agent:
  - **Restore** — re-attach to the _same_ Claude conversation in its original worktree (`claude --resume <id>`, protocol system prompt re-applied) and carry on. An agent that was mid-work is nudged to continue; one parked on a question keeps its escalation, and your answer routes straight into it.
  - **Requeue** — drop that conversation and queue the same work again for a fresh agent, with a prompt that names the origin, the branch that may already carry commits, and the crashed agent's last progress note.
  - **Remove** — abandon the work. The branch and worktree are left exactly as they are, so nothing is lost from disk.
- Restore needs a session id, a worktree still on disk and a runtime that can resume — the PTY runtime pins a session id up front (`claude --session-id <uuid>`, persisted on the `agents` row), the stream-JSON runtime does not. Where it can't be offered the card says why, and requeue and remove are always available.
- A deliberate **kill from the cockpit stays dead** — it's already a decision, so it is never offered. Nothing is auto-restored and nothing is auto-discarded: there is no setting to skip the question.
...
Every PR and issue carries a per-row **watch / ignore** button in the cockpit's
...
the type default — **PRs opt-out** (worked unless `-ignore`), **issues opt-in**
(left alone unless `-watch`). An ignored PR is filtered out of the dispatch view — no CI
fix, base update, review-comment note, or merge — and an un-watched issue is never
...
PR; `POST /api/issues/:number/watch` (`{ watched: boolean }`) sets the `-watch`/`-ignore`
pair on an issue. Each writes
